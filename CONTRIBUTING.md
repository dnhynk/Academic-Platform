...
Dependency additions require an owner, license review, feature review, advisory path, and an explanation of why the dependency belongs inside its trust boundary. Every dependency change enters through that admission process as a reviewed commit; `.github/dependabot.yml` therefore opens no version-update pull request, and a security advisory is triaged through the same process rather than merged as an automatic bump. Git dependencies and insecure package tarballs are rejected by the dependency-free `node tools/source-preflight.mjs` structural lock parsers before any fetch/install/build/test and are rechecked by `pnpm security`; update the Cargo and pnpm source-encoding fixtures with any lock-format change.
