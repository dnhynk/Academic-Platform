...
This repository is the runnable foundation for a local-first Personal Academic · CS · Project OS. It implements a synthetic, throwaway Phase 1 local core: the daemon owns a plaintext SQLite profile, accepts one allowlisted signed fixture over current-user local IPC, persists canonical and vault state, builds disposable projections, and supports doctor, deterministic export, consistent plaintext backup, and verified restore into a new empty profile. An optional Tauri desktop runtime renders the synthetic shell and sends allowlisted commands to that daemon over local IPC. The default graph retains its network prohibition; no product HTTP transport, recorder, cloud connector or arbitrary-input workflow is enabled. Real or production data remains forbidden because ADR-002 is unaccepted, the default lane reports `storage_encryption=NONE`, and five-platform SQLCipher evidence is incomplete.
...
- `academic-contracts`: deterministic CBOR v4 encode/sign plus v1/v2/v3/v4 decode/verify, semantic v4 validation of returned writer bytes, Ed25519 verification over original bytes, source-aware typed byte equality, device/key/user identity binding, pure v1-to-v3 and v2-to-v3 upcasters that rewrite no historical byte, and executable Protobuf actor/relation round trips with the same RFC-variant UUIDv7 boundary.
...
- `academic-next-lecture`: `P2-L6`'s section 12.7 next-lecture preparation, built on `P2-L4`, `P2-N5` and `P2-G5`. **Seven places, and no eighth**: the extraction sentence is split on the document's own commas, compared with `ExpectedConceptSource::ALL` in order and as sets, and then every matched cell is removed and what is left is required to be separators — so `다음 title/slide` is one place and a document repunctuated for eight fails here rather than leaving the enumeration silently wrong. **An extraction is a candidate**: `ExpectedConceptClaim::extract` takes a `P2-G5` `Proposal`, the only value `adjudicate` produces, and takes **no** epistemic status — section 27.4's `AI_INFERRED` is an associated constant, and the whole set of public signatures naming `EpistemicStatus` is exactly one accessor while no signature returns an `academic_knowledge_state` type at all. **The material is bound to the place that names it**: a claim none of whose cited spans point into its declared document is refused, and the seventh place — `직전 강의 말미` — requires a `P2-L4` `NodeId` that the other six are refused for carrying. **The minimum is three graph facts and no phrase**: a routed kind that is not `P2-N5`'s `강한 부족`, a concept no descent from tomorrow's lecture reaches, and a concept the graph puts **above** it; an unrelated topic answers with one defect and an advanced survey with two, so the refusal is attributable without either phrase existing here, and the whole method inventory of the package is pinned because a text comparison is the only way a phrase list could be used. **Three uncertainties are three types**: `ExpectedConceptReading`, `PrerequisiteEdgeReading` and `UserStateReading` share no trait and no collection, `PrepUncertainty` answers with no confidence of its own, and every public signature returning a `ConfidencePermille` is classified by its owner — so the fold section 12.7 forbids has no operation to be folded by. **The morning's `1–3` is parsed twice and held by two crates**: out of section 4 and section 25.2 independently, required to agree, and compared with `P2-X2`'s own bound. `propose` refuses to rank rather than choosing three, which is section 25.2's own `자동 중요도 순으로 숨기지 않고`. It opens no file, opens no socket, **reads no clock**, persists nothing and adds no migration. It leaves no `§38` gate open and closes none. See [the next-lecture preparation contract](docs/contracts/next-lecture-preparation.md).
...
- `academic-readiness`: `P2-Y3`'s section 24.3 career readiness view, built on `P2-Y1`, `P2-Y2` and `P2-U3`. **The matrix is the default view because it is the only one**: `ReadinessView::of` takes no mode, no preference and no flag, `render` emits the matrix as block zero in every view the crate can produce, and the whole set of public functions returning a view is compared against the four it documents. **Section 24.3 states two different sixes and this crate keeps them apart**: its table columns — `학문적으로 배움`, `문제/과제`, `Project 적용`, `장애/Debug`, `설계 선택`, `Freshness` — are corroborated key for key by section 36.9's own career-view block, and its prose's six evidence stages are `P2-Y1`'s `EvidenceStage`. Both are parsed back out of the design document and compared in both directions, and the two sets are required to stay **unequal** and to overlap on exactly `설계 선택`, so the open reading is executed rather than assumed. **No function maps a depth to a column** — that map would have to invent the `설계 선택` column's answer, one layer above where `P2-Y1` refused the same map — so the column is an argument and the record's own stage travels beside it. **No aggregate percentage is the primary output**, stated as absences: block zero is the matrix positionally, the block vocabulary is closed in both directions, and **no `f32` or `f64` is declared anywhere in the crate**, so a ratio has no type to arrive in whatever it is called. **A score does not exist without its four disclosures**: `disclose` is the one producer, takes all four by value, has **no score parameter** — the number is computed from the disclosed weights over the disclosed matrix — and re-derives the rubric, the sources and the missing data and refuses any that disagrees, so a disclosure of another matrix is refused rather than published beside it; a weighting that leaves a column out is refused rather than defaulted to zero. **Missing, unknown and freshness are three types**, with no conversion between the cell and the band in either direction — which matters because `FreshnessBand::Unknown` and `AxisCell::Unknown` are spelled the same and mean different things. **Every one of section 24.4's four directions terminates**: `traverse` takes one typed `StartingPoint` whose arm *is* the direction, a `Termination` holds its first terminus by value so an empty walk is not a value, and neither terminus arm carries free text, so `직무에 중요` has no field to arrive in. **Hiding a score and resetting a weighting are append-only**: both take `&self` and return a new view, nothing in the crate takes `&mut self`, and the prior number and weighting stay openable in the new view's history. **The non-guarantee notice survives export**: it has one producer and no argument, and `non_guarantee_disclaimer_survives_export` writes a real `P2-P1` bundle, reads it back with **no key and no vendor**, byte-compares the restored line, refuses the restored bytes with the notice removed and with it altered, and shows that deleting it from the published bundle makes `read_bundle` refuse the bundle. It opens no file, opens no socket, **reads no clock**, persists nothing and adds no migration, and inventories all 69 fields of every type it declares in both directions. Its atlas-domain tests are structural; the optional desktop runtime renders the shell frame and does not establish an interactive atlas performance claim. It leaves no `§38` gate open and closes none. See [the career readiness matrix](docs/contracts/career-readiness-matrix.md).
- `academic-home`: `P2-X2`'s section 25.2 `Home / Today`, built on `P2-X1` and `P2-G6`. **The order is not a layout accident**: `home_group_order_is_stable_one_to_eight` parses section 25.2's numbered list out of the specification and compares three enumerations that are not derived from one another — the document's own lines, `HomeGroup::ALL`, and `HomeGroup::position` — as sets in both directions *and* position by position on the document's own text, then rotates a corpus through every one of its positions and requires the rendered section sequence not to move. **The four permission words are closed by the type and are `P2-G6`'s five statuses, mapped**: `RecordingPermission` has no `FromStr`, no `TryFrom<&str>` and no free-form arm, and `RecordingPermission::of` names all five of `academic_consent::CaptureStatus` — which is `#[non_exhaustive]`, so the compiler cannot hold the sixth-status case and a source-level comparison against `CaptureStatus::as_str`'s own arms does, with `확인 필요` as the default-deny wildcard. **A prerequisite without a reason and a time is not constructible**: both are parameters of the one constructor, the reason is an `UpcomingUse` rather than prose, and the `1–3` bound is split out of `최대 1–3개` and used to judge the constructor at every count from zero to five. **Grouping cannot hide**: `GroupedAlerts::group` has no parameter a limit could be and hands back borrowed slices, and the union of `Today`/`Soon`/`No deadline` is compared with the input as multisets in both directions, with a deliberately lossy grouping beside it required to fail. **A freshness alert needs a use**: `FreshnessAlert::raise` takes an `UpcomingUse`, whose one constructor refuses an occasion that is not ahead of the reference instant, and the boundary is swept rather than sampled — which is what keeps `P2-N3`'s discipline visible on the first screen. **No hero metric, proved by exhaustion and with no forbidden-name list anywhere**: the card set against the group set, every field position against a reviewed inventory, the section sequence, and the shell's eight, each in both directions. It opens no file, opens no socket, **reads no clock**, persists nothing and adds no migration. It leaves no `§38` gate open. See [the Home / Today contract](docs/contracts/home-today.md).
- `academic-dashboard`: `P2-X3`'s sections 25.4, 25.5 and 25.6 — the four `Academic` routes — built on `P2-X1`, `P2-A3`, `P2-U1`, `P2-U4` and `P2-U8`. **Section 25.4 names four audit states and the engine publishes five, and that is recorded rather than resolved**: `ProofStatus` has `SATISFIED`, `NEEDS`, `NOT_SATISFIED`, `UNKNOWN` and `CONFLICT`, section 25.4 names `SATISFIED`, `REMAINING`, `UNKNOWN` and `CONFLICT`, and `REMAINING` is therefore the word both `NEEDS` and `NOT_SATISFIED` are shown as — forced by three of the four sharing a spelling and by the display having to be total, not invented. The collapse never reaches the value: `AuditStateReading` is built only from the engine's status and `engine_status()` is always available, so two readings that show one word are different values. **Every enumeration is parsed out of the specification and none is counted**: the three averages off the line's own middle dot, the four states off its back quotes with the residue required to be punctuation, the six timeline facets and the four coverage tabs off their own slashes, the six planner axes and six dashboard lines off their own bullet runs, and the seven §38 identifiers off each line's position in section 38. **A percentage is unrepresentable without its breakdown**: `SecondaryPercentage::over` is the only producer, takes the breakdown by value, computes the number rather than accepting one, and refuses a part reading `UNKNOWN` or `CONFLICT` — the same refusal `GpaValue::Unknown` makes one surface over. **The timeline reads two sources because `예정` is not an attempt**: `academic-record` has no constructor producing a `PLANNED` attempt, so the suite builds the ledger-only timeline and observes that facet constant before reading the two-source one, then requires every facet to vary, no two to agree on every row, and an appended repeat to leave the earlier row identical. **The planner re-reads six axes and never accumulates**: placing a candidate moves all six, and removing it returns the earlier outcome exactly. **A saved plan does not move**: `restate` takes `&self`, returns only a marking, and no `StaleInput` arm carries a replacement. **There is no registration route, and the crate links the crate that owns one so the absence is a measurement**: no product file spells `RegistrationConfirmation` or `SettledStatus`, the two planner files spell no ledger name at all, and every occurrence of the two `timeline.rs` reads is preceded by an ampersand. `GATE-38-001`–`GATE-38-006` surface as blocking dashboard inputs and `GATE-38-017` stays open every term. It opens no file, opens no socket, **reads no clock**, persists nothing and adds no migration. See [the academic dashboard contract](docs/contracts/academic-dashboard.md).
- `academic-cs-map`: `P2-X5`'s section 25.3 CS map and section 26 visualization system, built on `P2-X1`, `P2-N6`, `P2-N7`, `P2-C6` and `P2-Y3`. **The first screen is not the graph**: `initial_view` returns the field clusters and the goal's one-hop neighbourhood and refuses a graph outside section 25.3's own `10–20` — read out of the document rather than written here — instead of trimming to twenty. **Zoom is a semantic level, not a scale**: the four section 26.1 rows are parsed out of the table and compared position by position, the four type sets are pairwise different and nested in neither direction, the four node sets are pairwise different, and every node present at two levels sits at the **identical** coordinate in both. **Layout stability is a golden coordinate test**: `lay_out` takes a graph and nothing else — no lens, overlay, focus mode, zoom level or instant is a parameter — the sixteen landmark coordinates are pinned in `testdata/cs-map/landmarks.expected`, 829 views are exercised and every placement compared for exact equality afterwards, and the drift bound is required to be **attained** by the widest size pair so it is known to be tight rather than generous. **Opacity encodes lens relevance and never mastery**, held four ways and none of them a rule: two types; the whole set of 30 `impl` headers pinned in both directions and required to name **no** conversion trait for any type pair, which is the `From`-impl bypass `P2-Y3` measured; the whole set of 87 public signatures pinned, with the relevance producers compared against two and the signatures naming both a relevance and a mastery compared against the empty set; and six mastery levels drawn with everything else fixed, required to move the fill six ways and the opacity none. **Dashed is inferred and solid is confirmed** over all nine of section 30.2's statuses crossed with all twenty of section 7.2's predicates, with `Disputed` and `Superseded` placed on the dashed side and said so. **A third overlay is refused and leaves nothing behind**: `overlay` takes `self` by value, and the whole `base × overlay × overlay × overlay` product — ten thousand orderings — is enumerated with each verdict re-derived in the test. **Which channel a lens claims is measured, not decided**: each lens's own spec name is searched in each section 26.2 bullet in both directions, and the specification's own example composition turns out to collide, so that is the case the collision test runs. **`YOU` is not an ontology node**, checked by sweeping twenty-one produced surfaces each required to be non-empty first. **The scrubber reads both bitemporal coordinates** and agrees with `tools/cs-map-scrubber-oracle.mjs`, an independent transcription in another language with its own identities and its own algorithm; `user scope change` is the view-level transition `P2-C6` deliberately refused as a `ChangeOrigin`, and it carries none. It opens no file, opens no socket, **reads no clock**, persists nothing and adds no migration. **No Tauri runtime is linked and no window opens** for any claim it makes, and the five-thousand-node budget is a count budget rather than a stopwatch. It leaves no `§38` gate open and closes none. See [the CS map atlas contract](docs/contracts/cs-map-atlas.md).
...
All four committed signed fixtures are synthetic and declare `network_egress: NONE`. `signed-batch-v1.json`, `signed-batch-v2.json` and `signed-batch-v3.json` are byte-immutable read-only compatibility goldens; their original signed payloads are verified before a deterministic, fail-closed semantic upcast to the current model, and no public API or CLI can mint v1, v2 or v3. `signed-batch-v4.json` is the only writer fixture and adds the separate provenance-bearing deterministic prediction actor. It retains coverage of all eighteen event schema v3 registration arms at Proto tags 16..=33 — `CURRICULUM_VERSION_PUBLISHED`, `COURSE_REVISION_PUBLISHED`, `OFFERING_OBSERVED`, `ATTEMPT_RECORDED`, `REQUIREMENT_SET_PUBLISHED`, `AUDIT_COMPUTED`, `CAPTURE_PERMISSION_RECORDED`, `LECTURE_SESSION_RECORDED`, `TRANSCRIPT_VERSION_ADDED`, `LECTURE_DOCUMENT_PUBLISHED`, `SNAPSHOT_REGISTERED`, `FINDING_PUBLISHED`, `MODEL_RUN_RECORDED`, `PROPOSAL_DISPOSED`, `EGRESS_DECIDED`, `CONSENT_RECORDED`, `ENTITY_IDENTITY_CHANGED`, and `RETENTION_ACTION_RECORDED` — with the optional provenance digest present on some arms and absent on others, and it carries forward the v2 claims that demonstrate that a later AI inference cannot override an explicit user decision, that freshness can become `STALE` while mastery remains `PRACTICED`, and that a Prediction retains confidence plus versioned evidence-window/sample metadata distinct from its applicability interval. Fixture-wrapper ingress consumes original bytes with fatal UTF-8 decoding, so malformed sequences never become U+FFFD before JSON, Ajv, or semantic parsing. The raw boundary also rejects duplicate decoded property names, non-Unicode-scalar strings, and mathematically fractional designated integer lexemes at arbitrary precision while preserving integral spellings such as `2.0` and `2e0`. Prediction disclosure integers follow that same lexical rule while enforcing their typed bounds; every disclosed timestamp is a non-negative JavaScript-safe integer, and an applicability `to` key is required with explicit `null` as the sole open-ended representation. Artifact JSON applies the same ambiguity checks plus canonical unsigned number tokens, and Ajv, TypeScript, and Rust run the shared byte, exact-integer, prediction-metadata, and raw corpora.
...
cargo test -p academic-dashboard --test compile_fail --locked --offline
...
cargo test -p academic-next-lecture --test compile_fail --locked --offline
...
pnpm fixture:verify:v3
pnpm fixture:replay:v3
git diff --exit-code -- schemas/fixtures/
```

Hosted CI materializes **27 required jobs**: one source preflight, five
`rust-default-*` jobs, five `rust-store-*` jobs, five `rust-features-*` jobs,
five optional desktop runtime jobs, two Phase 1 exit jobs, three Linux-only encrypted/rotation jobs, and one pnpm
contracts job. A green run is therefore reported as **27/27**, and the measured
...
`P2-X1` fixes the route, palette, backlinks, drawer, typed allowlist and receipt boundary. `P2-X1b` now binds those contracts to an optional native Tauri window under the approved runtime gate. It embeds local HTML/CSS/ES modules and exposes one capability-scoped, versioned invoke command, with explicit unavailable, locked and incompatible states. The daemon remains the only canonical writer; the desktop links no store or key owner. Tauri 2.11.5 and tauri-build 2.6.3 add 327 registry entries only behind `desktop-runtime`; the default graph remains subject to the original network prohibition. See [the desktop shell contract](docs/contracts/desktop-shell.md) for executable launch and smoke instructions.
...
After bootstrap, build the embedded assets and select the isolated native feature:
...
Omit `--smoke` for the ordinary shell; pass `--session <daemon-runtime>/academic-os/<profile>/session.meta` to connect to an existing synthetic daemon. No session opens an explicit unavailable state. Saving becomes saved only after a matched core receipt; the receipt ID is available under Save details. See [the desktop contract](docs/contracts/desktop-shell.md) for prerequisites, command limits and native evidence.
