...
const BINARY: &str = env!("CARGO_BIN_EXE_academic-capture-client");
const CLASS: ProcessClass = ProcessClass::CaptureClient;
...
fn this_binary_is_bound_to_exactly_one_process_class() {
    ...
    assert!(
        source.contains("const PROCESS_CLASS: ProcessClass = ProcessClass::CaptureClient;"),
        "src/main.rs no longer binds CaptureClient"
    );
    assert!(
