...
use crate::{Core, service::AcceptanceService};
...
use academic_store::{
    ...
    queries::{DomainHistoryRequest, DomainHistorySnapshot, QueryError, domain_history_snapshot},
};
...
impl DetailContext {
    ...
    fn domain_projection(
        ...
        let snapshot = ReadSnapshot::authenticate(source, &self.trust)?;
        if profile_identity(profile).map_err(|_| dto::ReadFailure::ProfileUnavailable)?
            != self.profile_id
        {
            return Err(dto::ReadFailure::ProfileMismatch);
        }
        let mut projection = Projector::new(&snapshot, service, &self.profile_id)?;
...
fn query_failure(error: QueryError) -> dto::ReadFailure {
    match error {
...
impl<'a> Projector<'a> {
    ...
    fn new(
        ...
        let policies = academic_projections::resolution::PredicatePolicies::new(
            POLICY_VERSION,
            entries.clone(),
        )
        ...
        Ok(Self {
            ...
            registry: dto::PolicyRegistry {
                ...
                policy_registry_hash: policies.canonical_hash().into(),
    ...
    fn evidence(&mut self, id: EvidenceId) -> ReadResult<dto::EvidenceRef> {
        ...
        let excerpt = match &item.locator {
            academic_domain::EvidenceLocator::TextBytes {
                ...
                && *end <= 65_536 =>
            {
                let capability = self.service.vault().verify_sealed_object(&descriptor);
...
mod tests {
    ...
    fn accepted_claims_keep_exact_text_scope_authority_and_original_coordinates() -> TestResult {
        ...
        for (scope, expected) in [(4, "  CONFIRMED\nreported label  "), (6, "other scope")] {
            ...
            let mut projector =
                Projector::new(&snapshot, &fixture.service, fixture.context.profile_id())
                    .map_err(|r| format!("{r:?}"))?;
