...
use academic_lecture_document::{
    COVERAGE_CONFIG_V1, CaptureExclusionLedger, CoverageInputs, CoverageReport, CoverageValidator,
    CrossReference, DispositionLedger, DocumentAnnotation, DocumentBuilder, DocumentId,
    LectureDocument, NodeDraft, NodeId, NodeKind, PreservationTransform, RenderQa, RenderQaReport,
    RenderedImage, RenderedNode, RenderedPage,
};
...
/// A capture with one unexplained hole of five seconds.
pub fn capture_with_hole(
    ...
    recorder.record_audio_chunk(&mut ledger, chunk("first"), 0, INSIDE)?;
    recorder.record_audio_chunk(&mut ledger, chunk("second"), SECOND, INSIDE)?;
    recorder.record_audio_chunk(&mut ledger, chunk("third"), 6 * SECOND, INSIDE)?;
    recorder.capture_image(
        &mut ledger,
        image("board"),
        Orientation::TopLeft,
...
/// A capture whose timeline is broken by a resource failure and then resumed.
///
/// The failure writes a `Gap { ResourceFailure }` and stops the session; the
/// resume writes a `Gap { SessionResumed }` and starts a new clock. The two
/// audio frames on either side of that pair are on different clocks, so the
/// hole between them has no measurable length and is always a finding — and
/// the journal explains it.
pub fn capture_with_explained_gap(
    directory: &tempfile::TempDir,
    tag: &str,
) -> Result<Capture, Box<dyn Error>> {
    let mut ledger = ledger_permitting()?;
    ...
    recorder.record_audio_chunk(&mut ledger, chunk("first"), 0, INSIDE)?;
    recorder.record_audio_chunk(&mut ledger, chunk("second"), SECOND, INSIDE)?;
    recorder.observe(starved_reading(), 2 * SECOND, 2 * SECOND)?;
    // The resume reopens the same file, so the journal header still names the
    // capability token the first `begin` minted -- which is why the binding
    // below is opened against the original recorder and not the resumed one.
    let (mut resumed, _recovery) = academic_capture::resume(
        &mut ledger,
        &request()?,
        &path,
        &book,
    ...
    resumed.record_audio_chunk(&mut ledger, chunk("third"), 0, INSIDE_LATER)?;
    resumed.capture_image(
        &mut ledger,
        image("board"),
        Orientation::TopLeft,
        SECOND,
    ...
    let recovery = resumed.verify_on_disk()?;
    drop(resumed);
    Ok(Capture { recorder, recovery })
}
...
/// A clean render measurement of one document: one page, every node measured,
/// every placed capture resolved.
pub fn clean_render(document: &LectureDocument) -> Result<RenderQaReport, Box<dyn Error>> {
    ...
    let nodes: Vec<RenderedNode> = document
        ...
        .collect();
    let images: Vec<RenderedImage> = document
        .nodes()
        .iter()
        .flat_map(|node| node.nearby_captures().iter().copied())
        .map(|capture_frame_seq| RenderedImage {
            capture_frame_seq,
            resolved: true,
        })
        .collect();
    Ok(RenderQa::inspect(document, &pages, &nodes, &images)?)
}

/// Runs the validator over one document with no declarations and no exclusions.
pub fn validate(
    lineage: &TranscriptLineage,
    document: &LectureDocument,
    manifest: &InputManifest,
    journal: &JournalRecovery,
) -> Result<CoverageReport, Box<dyn Error>> {
    let dispositions = DispositionLedger::new();
    let exclusions = CaptureExclusionLedger::new();
    Ok(CoverageValidator::validate(&CoverageInputs {
        lineage,
        version: 1,
        document,
        manifest,
...
/// Runs the validator with declarations and exclusions supplied.
...
) -> Result<CoverageReport, academic_lecture_document::CoverageFault> {
    CoverageValidator::validate(&CoverageInputs {
        lineage,
        version: 1,
        document,
        manifest,
...
/// A cross-reference back to an earlier segment.
pub fn cross_reference(to: usize) -> CrossReference {
    CrossReference::to_segment(
        to,
        academic_lecture_document::CrossReferenceReason::InstructorReturnedToEarlierPoint,
    )
}
