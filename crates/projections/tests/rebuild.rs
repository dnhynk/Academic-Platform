...
impl Fixture {
    fn new() -> TestResult<Self> {
        ...
        let root = env::temp_dir().join(format!(
            "academic-projections-rebuild-{}-{sequence}",
            std::process::id()
        ));
    ...
    fn runner(&self) -> TestResult<ProjectionRunner> {
        ...
        Ok(ProjectionRunner::open(
            &canonical,
            &self.sidecar,
            ContentDigest::sha256(b"rebuild-test-builder"),
            ContentDigest::sha256(b"rebuild-test-config"),
        )?)
    }
...
fn projection_checksum_is_order_stable() {
    let records = [
        b"record-charlie".to_vec(),
        b"record-alpha".to_vec(),
        b"record-bravo".to_vec(),
    ];
    let expected = order_stable_checksum(records.clone());
    for permutation in [
        [records[0].clone(), records[1].clone(), records[2].clone()],
        [records[0].clone(), records[2].clone(), records[1].clone()],
        [records[1].clone(), records[0].clone(), records[2].clone()],
        [records[1].clone(), records[2].clone(), records[0].clone()],
        [records[2].clone(), records[0].clone(), records[1].clone()],
        [records[2].clone(), records[1].clone(), records[0].clone()],
    ] {
        assert_eq!(order_stable_checksum(permutation), expected);
    }
    assert_ne!(
        order_stable_checksum([b"ab".to_vec(), b"c".to_vec()]),
        order_stable_checksum([b"a".to_vec(), b"bc".to_vec()])
    );
}
...
fn as_known_rebuild_matches_ledger() -> TestResult {
    ...
    let historical = fixture
        .runner()?
        .build_as_known(ProjectionKind::Graph, domain, 1)?;
    let historical_again = fixture
        .runner()?
        .build_as_known(ProjectionKind::Graph, domain, 1)?;
    assert!(latest.activated);
    assert!(!historical.activated);
    assert!(!historical_again.activated);
    assert_eq!(historical.metadata.source_accept_seq, 1);
    assert_eq!(historical.metadata.source_outbox_seq, 1);
    assert_eq!(historical.metadata.record_count, Some(1));
    assert_eq!(
        historical.metadata.canonical_checksum,
        historical_again.metadata.canonical_checksum
    );

    let canonical = Connection::open(&fixture.canonical)?;
    let mut canonical_statement = canonical.prepare(concat!(
        "SELECT hex(c.claim_id) FROM claim c JOIN ledger_event e ",
        "ON e.event_id = c.assertion_event_id WHERE c.object_kind = 'ENTITY' ",
        "AND e.domain_id = ?1 AND e.accept_seq <= 1 ORDER BY c.claim_id"
    ))?;
    let canonical_claims: Vec<String> = canonical_statement
        .query_map([domain.as_bytes().as_slice()], |row| row.get(0))?
        .collect::<rusqlite::Result<_>>()?;
    drop(canonical_statement);
    drop(canonical);
    let sidecar = Connection::open(&fixture.sidecar)?;
    let mut projection_statement = sidecar.prepare(concat!(
        "SELECT hex(claim_id) FROM projection_graph_edge WHERE generation_id = ?1 ",
        "ORDER BY claim_id"
    ))?;
    let projected_claims: Vec<String> = projection_statement
        .query_map(
            [historical.metadata.generation_id.as_bytes().as_slice()],
            |row| row.get(0),
        )?
        .collect::<rusqlite::Result<_>>()?;
    assert_eq!(projected_claims, canonical_claims);
    drop(projection_statement);
    drop(sidecar);

    let active = fixture.reader()?.graph_neighbors(domain, entity(21)?)?;
    assert!(matches!(
        active.availability,
    ...
    assert!(
        active
            .records
            .iter()
            .all(|edge| edge.generation_id == latest.metadata.generation_id)
    );
    Ok(())
}
