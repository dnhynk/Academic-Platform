...
/// delimiter. Here it is what makes an `impl` header one string whatever the
/// formatter did to it, so `impl core :: fmt :: Display for X` and
/// `impl core::fmt::Display for X` are the same inventory entry.
///
/// It closes exactly that gap and nothing wider. Deleting **all** whitespace
/// joins unrelated tokens and can hide a name, which is worse than the hole it
/// closes.
