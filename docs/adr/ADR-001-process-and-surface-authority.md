...
The Unix endpoint path is bounded by the platform socket address, which carries 104 bytes on macOS and 108 on Linux including the terminator. The runtime root a caller supplies must therefore leave room for the fixed `academic-os/{profile}/d.sock` suffix below it, 36 bytes, which admits a runtime root of up to 67 bytes on macOS and 71 on Linux and so covers the ordinary macOS locations: a canonicalized `$TMPDIR` is 56 bytes and `~/Library/Application Support` is comparable. Startup measures the assembled path and refuses a root that does not fit, naming the limit, the measured length, and the path, instead of shortening the path or letting `bind` fail without a stated cause. `{profile}` is the leading 64 bits of a SHA-256 digest over the canonical profile root, so one profile always resolves to one endpoint and one lock, and two profiles share them only on a digest-prefix collision, whose probability across the `n` profile roots a single user hosts is about `n^2 / 2^65`. That is a namespacing choice inside one user's runtime area, not a security boundary.
...
The listener bounds what any one client can consume. Both frames of a connection are read under a deadline and the connection is closed when it expires, and the number of connections served at once is capped below the platform instance ceiling, so a client that connects and sends nothing cannot grow serve tasks, descriptors, or transport instances without limit. A transient accept error — a client that vanished between connect and accept, a momentarily full instance ceiling, descriptor pressure — is retried after a short pause rather than ending the listener, so transport backpressure degrades bounded service instead of becoming a silent, permanent loss of local IPC for every client. A Unix connection whose peer credentials do not match the current user, or cannot be read at all, is refused on its own for the same reason: macOS reads those credentials with `LOCAL_PEEREPID` and `getpeereid`, which need a still-connected peer and so return `ENOTCONN` for a client that closed before it was accepted, while Linux answers from what `SO_PEERCRED` recorded at connect. Neither outcome serves the connection, and neither ends the listener.
