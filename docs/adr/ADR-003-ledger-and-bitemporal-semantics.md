...
Prediction evidence time is a separate typed coordinate rather than a fourth bitemporal ordering axis. Every active `Prediction` carries confidence and versioned metadata with a bounded observation/history window plus a positive sample count; that window discloses the observations used to derive the prediction and never substitutes for `valid_time`. Signed decode, append, and resolution revalidate the requirement. `AiInferred` is unchanged and does not acquire a confidence or Prediction-metadata requirement through this rule.
