...
Fixture wrapper bytes pass a dedicated raw-text boundary before JSON parsing, Ajv, or TypeScript semantics. It rejects duplicate decoded property names and non-Unicode-scalar strings without imposing Artifact JSON's canonical-number profile. A shared v1/v2 raw corpus runs through Rust typed deserialization, Ajv plus the raw gate, and TypeScript. Rust uses bounded integral-number deserializers for `fixture_version` and `event_schema_version`, so equivalent integral decimal/exponent lexemes such as `2.0` and `2e0` remain valid while fractions, negatives, and values above `u16` fail.
...
Event/Proto v2 carries explicit semantic resolution-slot, target-object, and decision-validity fields rather than inferring them from a transient claim ID. The v1 reader performs the one deterministic compatibility inference described above. Its private source-version projection requires a private source-equality capability constructed only by authenticated read verification; exact public-API allowlisting, writer call-graph enforcement, downstream compile-fail imports for both the historical and a renamed projection, and executable rename/data-flow mutations prevent it from becoming a signer or caller-facing encoder. The frozen v1 fixture is checked against its committed exact document rather than regenerated.
