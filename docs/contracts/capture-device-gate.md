...
| C-12 | **No shipping process opens a device through this crate.** `academic-capture-gate` has no dependent: `rg -l "academic-capture-gate" crates/*/Cargo.toml` returns its own manifest and nothing else, and the one call to `academic_capture_gate::native::launch` in the workspace is in this crate's own `tests/native_device.rs`. `academic-capture-client` — the process class that declares `CaptureDevice` — links `academic-policy` and `academic-process-sandbox` and nothing else. So "no recording path bypasses permission" is today a statement about types and about a backend measured by a test harness on both hosts, not about a running recorder. | The task that opens a device from a shipping binary. `an_elsewhere_basis_is_linked_or_unreachable` is what holds the two apart until then: it passes because nothing in `academic-capture-client`'s resolved closure can exercise the capability, and it fails the day one can, which is the day this row has to be answered rather than restated. |
