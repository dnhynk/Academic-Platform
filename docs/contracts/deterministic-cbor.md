...
Before the additive v4 actor profile below, event schema v3 was the only writer profile, represented by `signed-batch-v3.json`, the v3 fixture JSON Schema, and `schemas/proto/academic/v3/ledger.proto`. It adds eighteen `OriginEvent.payload` arms at Proto tags 16..=33 over the frozen 10..=15, leaving 6..=9 reserved. The contracts crate validates the schema version decoded from the exact returned writer bytes, inventories every Rust source module, and pins every public function and inherent-method signature. Production-facing contracts/core APIs and the CLI expose no v1 or v2 signing or emission path. Each legacy source version owns a private projection that exists only for final source-aware byte equality, and core accepts the committed v1 and v2 fixtures as exact frozen-golden verification rather than regenerating them. Every identifier decoded from signed CBOR or Protobuf must be an RFC-variant UUIDv7, matching JSON Schema and TypeScript.
