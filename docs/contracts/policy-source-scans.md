...
| S-26 | `academic_audit::RuleSourceSpan::source_digest`, against `academic_requirement::ExecutableRule::source_digest` | The span's own documentation calls it *the byte span of the paragraph inside the official snapshot whose digest the rule already carries*, and nothing compares the two. `DegreeAudit::assemble` takes the span out of `AuditFacts::sources` by rule identifier and never reads the rule's digest at all, so a leaf can cite a paragraph of a document its rule was not read from. Measured on this tree: an index built with a digest no published rule rests on gives `DETERMINATE NOT_POSSIBLE`, **no outstanding check**, and thirteen leaves citing that other snapshot. It is `S-24`'s sentence one step out — the correspondence between two recorded digests is the caller's assertion — and it is section 11.3's citation rather than section 8.4's applicability, which is why `P2-RF26` recorded it instead of closing it. | Now, for any consumer that trusts a leaf's citation. Making the span underivable needs `AuditFacts::decode_sources` to carry the rule set, which it does not; refusing it needs a new `MissingCheck` arm and a rule that goes unevaluated. No fixture in this repository builds a mismatched index, so nothing regresses today. Severity **P2**. |
