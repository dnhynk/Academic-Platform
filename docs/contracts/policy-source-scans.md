...
whitespace-collapsed, and its literal values, compared to a constant — refuses
every edit to that item instead, including edits nobody predicted. Two things
it deliberately does not refuse, both measured rather than assumed: a comment,
and whitespace. Both views are whitespace-collapsed, so a reflow moves no key,
which is what makes a key that does move worth reading. The literal half was
added by `P2-RF31` after `P2-A5`'s sixth audit exchanged one domain-separation
constant for another with the whole workspace green.
...
| `keystore_leaf_public_facade_exposes_no_raw_handle` — `crates/keystore-platform/tests/facade.rs` | none — one fixed path, `../src/lib.rs` | no public signature in the FFI leaf names a raw handle, pointer, descriptor, key serial or D-Bus object; the three entry points are present; all three platform modules are declared private | `surface.len() >= 8` public signatures |
| `tools/macos-keystore-evidence.test.mjs` | constructed compiler records and a disposable validator bundle | not a product source-text scan: source-path literals describe synthetic compiler records; the test checks exact source/feature/target correspondence and rejection of substituted unsigned evidence | the complete three-target leaf set and separate crypto native harness; a fixture is never native execution evidence |
...
| `every_declaration_and_impl_in_this_crate_is_pinned`, `no_float_reaches_the_gpa_path`, `the_published_average_is_rounded_in_one_pinned_place`, `every_value_the_rule_book_renders_is_separator_free`, `the_rendering_separates_on_characters_no_identifier_can_hold` — `crates/record/tests/record_scans.rs` | recursive, the whole `crates/record` package less `tests`, so `examples/` and any `benches/` are read | not a token list: a float *type* under any spelling, a decimal-point literal, and an exponent literal, over code with comments and string literals removed; five evasion samples are run through the check inside the test and each must be caught; plus `every_declaration_and_impl_in_this_crate_is_pinned`, a whole-package walk (everything outside `tests`, so `examples/` and `benches/` are read too) comparing **every function declaration as a visibility and a full signature** and **every `impl` block header** against two pinned lists in both directions; plus the two that hold the rule-book hash's injectivity — `every_value_the_rule_book_renders_is_separator_free` compares each of five rendered types' **whole declared field set** against a rendered/unrendered partition and requires every rendered field's declared type to be one of eleven that cannot carry a separator, so a rendered `String` fails whatever it is called; and `the_rendering_separates_on_characters_no_identifier_can_hold` reads the `format!` templates out of the renderers and requires the character on each side of every `{}` to be one `CanonicalIdentifier::new` refuses, so a separator the charset admits and a charset widened to admit a separator both fail | `>= 11` files, plus a tripwire that every `pub mod name;` in `lib.rs` is a file the walk read; the inventory walk floors at 15 files, 224 declarations, 55 impl headers; the field reader is required to still report a declared `String` where one exists, `>= 12` rendered fields must be classified, and `>= 6` templates and `>= 16` interpolation boundaries must be read |
...
| `no_numeric_source_winner`, `credentials_never_reach_a_general_crawler`, `no_captcha_or_access_control_bypass_module_exists`, `the_only_public_route_to_snapshot_bytes_is_the_untrusted_seal`, `the_walk_reads_every_module_in_this_crate`, `this_crate_declares_three_product_edges`, `every_declaration_and_impl_in_this_crate_is_pinned` — `crates/ingestion/tests/ingestion_scans.rs` | recursive, **every `.rs` under the whole `crates/ingestion` package** for the per-crate rules, less `tests` for the ones about shipped code; recursive over **every `.rs` under `crates/`** for the workspace-wide inventory; fixed paths for the pins | whole-set comparisons rather than token lists: the item set of `conflict.rs` at file scope, the **owner-resolved** signature set of every function anywhere in the crate that touches a conflict value, a credential binding or a request — `Self` and the receiver are resolved against the enclosing `impl`'s subject before the type is looked for, which is the half `P2-A3` walked past by writing `Self`; the whole declaration set of the package as a file, a visibility and a full signature, beside the whole `impl` header set, so a new function, method, inherent `impl` or trait `impl` fails as an entry nobody wrote down whatever it is called, the public surfaces of `ConflictCase`, `ContendingSource`, `CredentialBinding`, `ConditionalRequest` and `RawSnapshot`, the whole external import set, the whole `Cargo.toml` edge set, four access vocabularies compared variant by variant, whole-text pins on `credential_binding`, `DeclaredTarget::declared` and `deny`, call-site counts for `credentialed`, `declared`, `source_bytes()` and the two `Denial` initialiser fields, and a three-shape numeric rule over `conflict.rs` — a numeric type under any spelling, a numeric literal under any spelling, and a counting/positioning vocabulary — with five evasions and four benign shapes run through it inside the test | `sources.len() >= 17` on the package walk, `declared >= 13` on the module tripwire, `naming.len() >= 5` on the workspace walk, `sources.len() >= 14` on the inventory walk; every whole-set comparison is an `assert_eq!` against a pinned list, so an empty walk fails as missing keys |
...
| `tools/desktop-runtime-policy.test.mjs` | exact optional Cargo closure and fixed desktop runtime/build/capability/client paths | full feature/dependency admission, no store/key owner or plugin, exact invoke manifest and local capability, bounded host metadata read and one mutation send site | exact closure equality and nonempty command/capability equality; native smoke complements source checks |
| `only_egress_crate_has_a_socket` — `tools/phase1-scaffold-policy.test.mjs` | recursive, **every `.rs` under every workspace package**, comments and every literal — raw strings included — stripped before matching | a per-file allowance of exact socket spellings (each IPC file and the two `P2-G4` files named individually, every other allowance empty); a rule that a crate root or a socket module segment may be renamed only to `_`, read on `use` and on `extern crate` alike; a rule that no file in the workspace may import `libc::syscall` — by name, under an alias, inside a braced list, or through `use libc::*` — so a call to it has to spell `libc::syscall(`; zero foreign-function declarations anywhere; every `#[path]` target resolved and required to be one of the files this scan read; the one `include!` pinned whole; a pinned build-script inventory; a per-crate link closure intersected with the socket-capable crates; and, for each of the two Linux seccomp backends, two rules over syscalls: every `libc::syscall(` call in the file must name a `libc::SYS_` constant as its first argument — optionally OR-ed with `X32_SYSCALL_BIT`, the one computed number this repository admits, and only where the file pins that constant's definition as whole text — and the constant must be one of that file's own reviewed calls, and every other `SYS_` spelling in the file must sit inside `denied_syscalls`, **counted** | `scanned.length >= 10` on the capability scan it sits beside; the allowance map is compared whole, so a file that stops being read fails as a missing key |
...
| `the_walk_reads_every_module_in_this_package`, `the_competency_crate_touches_no_file_and_no_socket`, `every_field_of_this_crate_is_in_the_inventory`, `no_public_function_mutates_in_place`, `the_rubric_is_configuration_and_not_a_constant`, `each_guarded_name_has_exactly_its_call_sites`, `the_helpers_are_not_vacuous`, `this_scan_is_in_the_inventory`, `every_impl_header_in_this_crate_is_in_the_inventory` — `crates/repository-competency/tests/competency_scans.rs` | recursive, **every `.rs` anywhere under the package**; the `use`/path/macro comparison reads `src` only, the forbidden-construct pass reads `tests` too | three whole-set comparisons in both directions — every `use` item, every two-segment path reached through a crate root, every macro invoked — plus an 11-token forbidden-construct pass as the third and weakest layer; a whole-set field inventory over all 100 declared fields in both directions, each carrying which of eight things it holds; a `&mut self` scan; the rubric-is-configuration scan, which reads `docs/contracts/repository-competency.md` as well as the source; and four counted call sites each pinned to its one file; and **a whole-set comparison of every `impl` header this crate declares** against a pinned inventory in both directions, which then refuses fifteen folding traits and every scalar conversion target over the whole inventory rather than over named type pairs — `P2-A5` injected `impl From<&PromotionSet> for u32` into `academic-repository-competency` and it passed 1543 tests over 265 binaries, because a trait `impl` declares no `pub fn` and no signature inventory can see one | `>= 9` files, `>= 6` module declarations each read by the walk, `>= 6` product files |
| `crates/repository-competency/tests/competency_lanes.rs` | one fixed read of `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md`, for section 17.6's five bullets and section 13.2's ceiling table | not a source-text scan: `P2-R5`'s seven named acceptance tests, plus `P2-RF19`'s `a_control_flow_edit_inside_vendored_source_bears_no_understanding` and `two_works_touching_one_observation_are_refused_rather_than_chosen_between`, over in-process synthetic corpora captured through `P2-R1`'s own `capture_local`, analyzed through `P2-R2`'s own ladder, correlated through `P2-R3`'s own `correlate` and classified through `P2-R4`'s own `classify`. It reads the design document so that section 17.6's bullet count and section 13.2's `mastery 승격 없음` ceiling are measured rather than restated | n/a |
...
| `every_declaration_and_impl_in_this_crate_is_pinned`, `the_walk_reads_every_module_in_this_crate`, `the_selector_dimensions_are_the_specifications_own`, `the_profile_fields_are_the_specifications_own`, `the_open_gates_are_section_38s_own`, `the_proof_statuses_cover_section_11_3s_own_tree`, `no_product_file_reaches_a_clock_rng_socket_or_model`, `no_product_file_names_a_projection_and_only_one_names_a_plan`, `the_three_witnesses_have_one_construction_site_each`, `the_only_defaults_are_empty_collections`, `the_scans_these_documents_enumerate_are_the_ones_this_file_holds` — `crates/audit/tests/audit_scans.rs` | recursive, every `.rs` under `crates/audit/src` | section 11.1's selector sentence, section 3's profile block, section 38.1's ten lines and section 38.2's eleven bullets and section 11.3's tree, each parsed out of the design document and compared in both directions; a fourteen-spelling clock/RNG/socket/model sweep with its evasions run through the same predicate inside the test; a whole-set sweep for a projection name and a two-file allowance for a plan name; three witness declarations, three crate-private `establish` sites, a pinned constructor and one determination expression; and the whole `#[derive(Default)]` set against a four-entry list; plus `every_declaration_and_impl_in_this_crate_is_pinned`, a whole-package walk (everything outside `tests`, so `examples/` and `benches/` are read too) comparing **every function declaration as a visibility and a full signature** and **every `impl` block header** against two pinned lists in both directions; plus `the_scans_these_documents_enumerate_are_the_ones_this_file_holds`, which reads this file's own `#[test]` functions and compares them against the number word and the two enumerations on this page and against the number word in [graduation audit](graduation-audit.md), in both directions | a `pub mod` tripwire — every module `lib.rs` declares must be a file the walk read, and the walk must read no file no module declares — plus `declared >= 12` and `sources.len() >= 12`; the inventory walk floors at 14 files, 265 declarations, 55 impl headers; the scan-count reader is required to find itself among the scans it read |
...
| `the_walk_reads_every_module_in_this_package`, `the_competency_crate_touches_no_file_and_no_socket`, `every_field_of_this_crate_is_in_the_inventory`, `the_unnamed_fields_are_pinned`, `no_public_function_mutates_in_place`, `the_statement_is_rendered_rather_than_stored`, `the_join_has_no_second_key`, `no_product_file_names_a_project_observation_claim`, `each_guarded_name_has_exactly_its_call_sites`, `the_helpers_are_not_vacuous`, `this_scan_is_in_the_inventory` — `crates/competency/tests/competency_scans.rs` | recursive, **every `.rs` anywhere under the package**; the `use`/path/macro comparison reads `src` only, the forbidden-construct pass reads `tests` too | three whole-set comparisons in both directions — every `use` item against a 27-entry inventory split into non-`crate` imports, `crate::` imports and `pub use` re-exports, every two-segment path reached through a crate root (one, `thiserror::Error`), and every macro invoked (four) — plus a 15-token forbidden-construct pass as the third and weakest layer, which carries `Instant`, `SystemTime` and a bare `now` beside the filesystem and transport names because *this engine cannot ask what time it is* is what keeps freshness `P2-N3`'s; a whole-set field inventory over all 55 declared fields in both directions, each carrying which of six things it holds and each refused a `u8` or a `u64` in its declared type, beside a separate rule over the four tuple structs and the three enumerations with unnamed variants, because a whole-set claim over *named* fields would otherwise miss them; two whole-text pins that are the two rules of this task — `Competency::statement`, which renders section 24.1's sentence from the parts, and `PerformanceCriterion::is_about`, which is the join's one key — beside `declare`'s whole signature (five arguments, none a sentence), a requirement that `sheet.rs` name `enabled_by` and `about` **zero** times, and a requirement that no product file name `ProjectObservationClaim` at all, which is `P2-R5`'s separation held one layer up; a `&mut self` scan; and two counted call sites each pinned to its one file | `>= 8` files in the package walk against `lib.rs`'s own `pub mod` list in both directions, `>= 7` module declarations, `>= 10` files in the whole-package walk, and controls on the forbidden-token pass — the same reader is required to find `File` and a bare `now` in samples written here, so the zeros it reports over this crate are measurements rather than a reader that always answers zero |
| `crates/competency/tests/competency_model.rs` | one fixed read of `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md`, for section 24.1's example competency, section 24.3's evidence-stage sentence and section 13.2's ceiling table | not a source-text scan: `P2-Y1`'s six named acceptance tests over in-process synthetic corpora captured through `P2-R1`'s own `capture_local`, analyzed through `P2-R2`'s own ladder, correlated through `P2-R3`'s own `correlate`, classified through `P2-R4`'s own `classify` and promoted through `P2-R5`'s own `promote`, beside knowledge-state items admitted through `P2-N2`'s own four eligibility checks. It reads the design document so that section 24.1's criterion, concept and rubric counts and section 24.3's six stage names are measured rather than restated | n/a |
| `the_walk_reads_every_module_in_this_package`, `the_role_profile_crate_touches_no_file_and_no_socket`, `every_field_of_this_crate_is_in_the_inventory`, `the_unnamed_fields_are_pinned`, `no_public_function_mutates_in_place`, `the_only_producers_of_a_bundle_are_the_three_doors`, `an_interest_is_not_an_input_to_anything`, `a_label_reaches_no_direction_and_no_single_bundle`, `the_identity_is_a_pair_and_the_rendering_has_no_inverse`, `the_identifier_rule_is_executed_over_every_byte`, `each_guarded_name_has_exactly_its_call_sites`, `the_helpers_are_not_vacuous`, `this_scan_is_in_the_inventory` — `crates/role-profile/tests/role_scans.rs` | recursive, **every `.rs` anywhere under the package**; the `use`/path/macro comparison reads `src` only, the forbidden-construct pass reads `tests` too | three whole-set comparisons in both directions — every `use` item against a 27-entry inventory split into non-`crate` imports, `crate::` imports and `pub use` re-exports, every two-segment path reached through a crate root (one, `thiserror::Error`), and every macro invoked (two, `format` and `matches`) — plus a 17-token forbidden-construct pass as the third and weakest layer, which carries `Instant`, `SystemTime` and a bare `now` beside the filesystem and transport names because *this crate cannot ask what time it is* is what keeps a bundle's `validAt` valid time rather than a freshness reading; a whole-set field inventory over all 57 declared fields in both directions, each carrying which of six things it holds and each refused a `u8` or a `u64` in its declared type, beside a separate rule over the six tuple structs and the three enumerations with unnamed variants, because a whole-set claim over *named* fields would otherwise miss them; four rules of this task, each measured rather than described — **the whole set of public functions returning a `RoleProfile` by value** must be exactly `declare`, `revise` and `fork`, which is `GATE-38-029` in the source because a shipped default bundle would be a fourth producer; **no public signature anywhere may name `RoleInterest`**, compared against `impl RoleInterest`'s own four functions, so a favourite cannot become an argument to anything; `direction.rs` must name `RoleLabel` **zero** times and `identity.rs` must name `RoleDirection` **zero** times, with `by_label`'s whole signature pinned, so a job title cannot be read as a direction or resolved to one bundle; and `RoleProfileRef`'s declaration and `rendered`'s whole body are pinned with no `FromStr` and no `TryFrom<String>`, so section 24.2's `_v4` spelling has no inverse; a `&mut self` scan over the package, `BundleShelf` included; and four guarded names, each with its call sites compared file by file | `>= 7` files in the package walk against `lib.rs`'s own `pub mod` list in both directions, `>= 6` module declarations, `>= 9` files in the whole-package walk, and controls on the forbidden-token pass — the same reader is required to find `File`, a bare `now` and `TcpStream` in samples written here, so the zeros it reports over this crate are measurements rather than a reader that always answers zero |
| `crates/role-profile/tests/role_bundles.rs` | one fixed read of `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md`, for section 24.2's YAML block, its direction sentence and its `importance` values, and for sections 25.11 and 37's two refusals | not a source-text scan: `P2-Y2`'s six named acceptance tests plus seven more, over in-process values whose competencies are `P2-Y1`'s own `CompetencyId` and whose dates are `P2-U6`'s own `Date`. It reads the design document so that four things are **measured in both directions** rather than restated — the seven keys of section 24.2's YAML block against this crate's wire, with the three deliberate differences named in the assertion; the two keys of its `competencies` entry; its three `importance` spellings; and its twelve direction names against `RoleDirection::NAMED`, beside the `등` marker that says the list is open. Sections 25.11 and 37's two refused standings are read the same way, each required to be absent from `InterestStanding::ALL` and present in the document that refuses it, so a specification that stops saying so fails here | exactly 12 names parsed from section 24.2's own sentence and 12 arms in `RoleDirection::NAMED`, an empty shelf required to report all 12 directions as uncovered, every one of the wire's nine keys removed one at a time and required to refuse, and all 256 single-byte values classified against the identifier rule in both directions |
| `the_walk_reads_every_module_in_this_crate`, `no_login_bypass_or_evasion_module_exists`, `raw_review_text_is_excluded_from_export_and_share`, `every_field_of_every_type_is_classified`, `each_value_has_one_producer` — `crates/review/tests/review_scans.rs` | recursive, **every `.rs` under `crates/review`** for the crate's own sets, and **every `.rs` under every `crates/*` package** for the two workspace-wide sets; plus a second recursive walk of `crates/export` for the bundle half. Comments and every literal — raw strings included — are stripped before any comparison, so a scan file's own quoted shapes are not read as code | no token list anywhere: seven whole sets compared with a committed table in both directions — every `use` statement in the crate, every function declaration as a visibility and a signature, every field of every type as `(type, field, field type, class)` with the class drawn from a six-word vocabulary, every `impl` header, every file that reads the retained text, every workspace file whose code names one of the six values an outbound request is composed from, every workspace file outside this crate naming one of its types or its crate path, and the whole set of functions producing a `CourseAggregate`, an `OfferingAggregate` or a `ReviewRecord`. Plus per-file assertions that no `crates/export` file names this crate and that no product file here reaches `serde` | `the_walk_reads_every_module_in_this_crate` requires at least fourteen files, requires the crate root among them, and requires every `mod name;` in every file to resolve to a file the walk read; every set comparison is against a non-empty pin, so an empty walk fails as a set of missing keys |
| `scalar_is_not_a_course_property` — `crates/review/tests/reviews.rs` | none — two fixed paths: `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md` for section 29.5's scope sentence, disclosure sentence, `ReviewRecord` block and access-mode union, and `crates/curriculum/src/course.rs` for section 8.2's `Course` field list | not a source-text scan in the policy sense: the `Course` read compares that struct's whole field list against the three fields section 8.2 gives it, in both directions, and requires every review-dimension spelling and every course-aggregate type name to be absent from that module | the field list is compared with `assert_eq!` against a three-entry expectation, so a read that returned nothing fails as three missing entries |
| `the_walk_reads_every_module_in_this_package`, `the_readiness_crate_touches_no_file_and_no_socket`, `every_field_of_this_crate_is_in_the_inventory`, `the_unnamed_fields_are_pinned`, `no_public_function_mutates_in_place`, `the_only_producer_of_a_score_is_disclose`, `the_matrix_is_the_only_view_and_it_is_first`, `no_function_maps_a_stage_or_a_kind_to_an_axis`, `a_cell_and_a_band_are_never_one_field`, `this_scan_is_in_the_inventory`, `the_helpers_are_not_vacuous` — `crates/readiness/tests/readiness_scans.rs` | every `.rs` under `crates/readiness/`, product tree and test tree | `P2-Y3`'s crate boundary. Three whole-set comparisons in both directions — every `use` item, every two-segment path reached through a crate root, every macro invoked — then a whole-identifier pass for seventeen filesystem, clock, process and transport constructs over every file of the package, tests included. The field inventory compares all 69 declared fields in both directions, requires each to say which of seven things it holds, and refuses `u8`, `u64`, `f32` and `f64` in any declared type, so neither a byte buffer nor a clock reading nor a ratio has a name to arrive under; `the_unnamed_fields_are_pinned` closes the two tuple structs and the five enumerations with unnamed variants that a named-field comparison would miss. Three task rules are pinned rather than described: the whole set of public functions producing an `AuxiliaryScore` **by value** is compared against `disclose` and its parameter list is required to name all four disclosures and no `ScoreValue`; the whole set producing a `ViewBlock` is compared against `render` and the first block that function names is required to be the matrix; and the whole set of public signatures that name both a `ReadinessAxis` and a `P2-Y1` stage or a `P2-N2` evidence kind **and return something** is required to be empty, with both halves of that conjunction shown to be separately non-empty and the predicate shown to bite on a fragment that does map one to the other | all 20 non-crate `use` items, all 7 `crate::` items, all 9 `pub use` re-exports and all 3 remaining items compared as one set; all 69 fields in both directions; the score producer set, the view-block producer set and the depth-to-column map set each compared against a pinned answer |
| `crates/readiness/tests/readiness_matrix.rs`, `crates/readiness/tests/readiness_export.rs` | one fixed read of `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md` for seven readings, and a walk of `crates/readiness/src/` for the scalar and mutation passes; `readiness_export.rs` also reads and rewrites the bundle it wrote under the build lane's temporary directory | not a source-text scan: `P2-Y3`'s eight named acceptance tests plus six more, over in-process values whose competencies are `P2-Y1`'s own `declare`, whose bundles are `P2-Y2`'s own `declare`, whose knowledge-state items passed `P2-N2`'s four eligibility checks and whose personal application claim was promoted through the whole `P2-R1`–`P2-R5` chain. It reads the design document so that **seven** things are measured in both directions rather than restated — section 24.3's six table columns, section 36.9's six career-view keys, section 24.3's six evidence-stage names, section 24.3's disclosure sentence, section 24.4's four direction bullets, section 34.5's career-readiness row and section 35's anti-goal row. The two sixes of section 24.3 are required to be **different sets** overlapping on exactly `설계 선택`, so the open reading recorded in [the career readiness matrix](career-readiness-matrix.md) fails here if the document stops supporting it. `no_primary_aggregate_percentage` walks the product source for a declared `f32` or `f64` and for every field of type `ScoreValue`, which is the absence half of that test; `score_hide_and_weight_reset_preserve_history` walks it for `&mut self` | six columns parsed from two independent places and compared position by position; four direction bullets compared in both directions; all 256 single-byte values classified against the identifier rule in both directions; every rendered block kind compared against `ViewBlock::KINDS` in both directions; every `ScoreValue` field compared against a four-entry answer; and `the_signature_scanner_is_not_vacuous` runs the signature scanner over a fragment whose answers are known, including the `-> Result<Self, _>` shape a naive scanner misses |
| `home_group_order_is_stable_one_to_eight`, `permission_status_is_exactly_four_values`, `no_gpa_or_streak_hero_component`, `the_home_surface_cannot_name_a_mastery`, `the_field_reader_finds_a_position_nobody_reviewed` — `crates/home/tests/home.rs` | recursive, the whole `crates/home` package outside `tests`, plus three fixed reads: `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md` for section 25.2's numbered list and its overflow sentence, `crates/consent/src/status.rs` for `CaptureStatus::as_str`'s arms, and `crates/knowledge-state/src/ladder.rs` as the mastery reader's control | whole sets in both directions and no forbidden-name list anywhere in the file: every field position of every type against a reviewed inventory that says which of section 25.2's eight lines each one serves; `HomeCard`'s variants against `HomeGroup`'s and against `HomeGroup::ALL`; the five capture statuses this crate names against `academic-consent`'s own arms; every `impl` header in the crate against a twenty-entry reviewed list, and every derived trait keyed on the type deriving it against a per-type map -- keyed, because a flattened set holds `Default` once for `HomeScreen` and an injected `Default` on `AlertBucket` passed it; six mastery spellings against every product file, with a control requiring the same reader to find at least four of them in `P2-N2`'s ladder | `product_sources` fails below seven files; the field reader fails below forty positions and the variant reader below eight; every `mod` in `lib.rs` must name a file the walk read; `the_field_reader_finds_a_position_nobody_reviewed` drives the reader over a fixture holding a struct field, a tuple position and a struct-variant field and requires all five to be found, and requires it to find nothing in a file with no type in it |
| `the_home_sections_are_the_crates_own`, `no_gpa_or_streak_hero_component` (shell half) — `packages/ui/src/home.test.ts` | none — two fixed paths, `crates/home/src/lib.rs` and `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md` | the arms of `HomeGroup::id`, `HomeGroup::position` and `HomeGroup::ALL` are parsed out of their own bodies — the matches rather than the enum, so an arm renamed with a stale identifier fails on the identifier — and compared with `HOME_GROUPS` as sets in both directions, in the crate's order, and against section 25.2's own numbering. The shell half of the absence claim compares the rendered `/` view's sections against the crate's own arms rather than against `HOME_GROUPS`, in both directions, and requires the first to be today's schedule -- against `HOME_GROUPS` an injected ninth entry passed, because both sides of that comparison were the same list | a raise if a signature or its closing brace is not found, `positions.size > 0`, a required single opening of the home route, and a negative control: an arm the crate does not have reads as absent |
| `the_walk_reads_every_module_in_this_package`, `the_blind_spot_crate_cannot_name_a_mastery`, `the_finding_has_exactly_one_producer`, `the_presentation_carries_no_free_text`, `no_goal_vocabulary_reaches_this_crate`, `no_clock_socket_or_file_reaches_this_crate`, `only_the_named_test_files_read_anything`, `the_blind_spot_decisions_are_pinned`, `every_public_signature_is_in_the_inventory`, `no_public_function_mutates_in_place`, `the_helpers_are_not_vacuous`, `this_scan_is_in_the_inventory` — `crates/blind-spot/tests/blind_spot_scans.rs` | recursive, **every `.rs` anywhere under this crate's package**, split into product source (everything outside `tests`) and all source; plus five fixed reads of this crate's own modules for the whole-text pins, one of its `Cargo.toml`, one of `docs/contracts/policy-source-scans.md`, and one of `crates/knowledge-state/src/ladder.rs` as the control | **four whole-set comparisons of what the product code can reach, all in both directions** — every `use` item against a 49-entry list, every two-segment path spelled through a crate root against a 5-entry list, every macro invoked against a 1-entry list, and the twelve modules `lib.rs` re-exports from against its own `pub mod` declarations; then the four rules those exist to hold. First, **a coverage reading cannot become a mastery score**: `P2-N3`'s eight mastery spellings are refused over every product file and again over every public signature, twelve score-folding spellings — `score`, `percent`, `percentage`, `ratio`, `mean`, `average`, `weight`, `weighted`, `normalise`, `normalize`, `proficiency`, `rank` — are refused as whole identifiers, and `FieldCoverage` and `EvidenceDiversity` are required to derive neither `PartialOrd` nor `Ord`, read out of the `#[derive(..)]` attribute immediately above each declaration. Second, **there is only one rerun to drive**: every public signature whose return type names `BlindSpotFinding` is compared against a one-entry pin in both directions, the private constructor's call sites against a two-file pin, and the ledger module is required to name none of `remove`, `clear`, `retain`, `drain`, `take`, `delete`. Third, **no text can enter the presentation**: eight warning spellings — `WarningRed`, `Severity`, `Alert`, `Danger`, `Critical`, `Urgent`, `Priority`, `Pressure` — are refused everywhere, both presentation types are pinned whole and required to own no `String` or `Cow`, and `headline` is pinned whole. Fourth, **no goal can be named**: eight goal spellings — `ActiveGoal`, `GoalCriteria`, `SuccessCriterion`, `GapCase`, `academic_gap`, `Objective`, `Remediation`, `Recommendation` — are refused over every product file, and a manifest rule with comment lines stripped first refuses an `academic-gap`, `academic-freshness`, `academic-store`, `academic-worker`, `academic-egress-boundary` or `academic-vault` edge. Above those four sits the comparison that closes what a name list cannot: **the whole set of this crate's public functions, as module, name and return type, against a 114-entry pin in both directions**, because five injections that spelled none of the names in any list above — a coverage fold, a demanding sentence, a ledger drop, a shipped scope and a goal reading — each passed every one of them. Then four whole-text pins on the decisions a later edit could move without any behavioural test noticing (`LOW_RECENCY_BANDS`, `state_of`, `EvidenceDiversity::of_distinct_sources`, `suppresses_warning_at`) plus the whole `detect` classification order as a whitespace-collapsed constant, three substring guards inside `FieldCoverage::of` — the misattribution `P2-N2` found one layer up and `P2-N3` one hop out — and two inside `TastePath::for_explore`; a refusal of any `pub fn` taking `&mut self`; a whole-set comparison of the product files naming `Default` against a one-entry pin, which is what makes `taxonomy granularity와 기간을 사용자가 선택한다` unskippable; and, as a third and weakest layer, a forbidden-token pass over every product file for fifteen filesystem, clock, process and transport spellings — `std::time` and a bare `now` among them, which is what makes *this engine cannot ask what time it is* a property of the whole crate | `>= 13` modules in the package walk against `lib.rs`'s own `pub mod` list in both directions, `>= 13` files scanned by the forbidden-construct pass, `>= 110` public functions in the signature inventory with at least one numeric return among them, `>= 10` public functions found in `finding.rs` so the producer filter is not matching nothing, a two-entry pinned set of the files of this package permitted to read anything, and **a control on the mastery rule**: the same reader is required to find at least five of the eight names in `P2-N2`'s own `ladder.rs`, so the zero it reports here is a measurement rather than a reader that always answers zero; beside it every extractor is re-exercised against a sample it must match, each of the eight mastery, twelve score, eight warning and eight goal spellings is required to be found in a sample that does spell it, the manifest reader is required to see the two edges this crate does have, each whole-text pin is required to extract more than 100 characters, and a signature, a type declaration and a `const` the extractor cannot find are each required to be an error |
| `crates/blind-spot/tests/blind_spot.rs` | one fixed read of `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md` per case | not a policy scan. Section 23's five-state block, its coverage sentence, its schema block, its four back-quoted disposition names, its three em-dash-delimited taste steps and the two halves of its `약하다`/`판단할 exposure가 없다` bullet are read back out of the design document and compared against `BLIND_SPOT_STATES`, `EXPOSURE_SOURCES`, `FINDING_FIELDS`, `DISPOSITIONS`, `TASTE_STEPS` and this crate's two copy constants in both directions, so five, five, eight, four and three are measurements rather than counts restated in a test. `low_relevance_uses_neutral_tokens` additionally compares the **whole set** of strings a finding can render against the document itself — six values, each required to occur verbatim in it — which is what refuses a demanding sentence whose words no list here holds | the schema example's own `exposureEvidenceCount` and `evidenceDiversity` are read and required to be reproduced by a reading built from one item; the example's `scope` string is required to be reproduced by the two halves the user selected; the example's `userDisposition` is read and required **not** to be one of the four the bullet enumerates; three demanding sentences spelled with none of the words any list here holds are required to be neither in the document nor renderable; and `not_relevant_survives_ai_rerun`'s five reruns carry a five-state control with an empty ledger, so the sweep measures the ledger rather than a detector that never changes its answer |
| `plan_scenario_never_writes_actual_state`, `no_mastery_delta_in_plan_output`, `no_default_recommendation_score`, `every_declared_field_position_is_reviewed`, `frozen_inputs_are_the_whole_of_what_the_engine_reads` — `crates/what-if/tests/what_if_scans.rs` | recursive, the whole `crates/what-if` package outside `tests`; plus one non-recursive listing of `crates/` for every workspace manifest, one read of this crate's own `lib.rs` for its `pub mod` list, and three fixed reads — `crates/knowledge-state/src/ladder.rs` and `crates/domain/src/lib.rs` for the mastery vocabulary, `crates/store/src/profile.rs` as the ambient-name control, and `crates/review/src/bias.rs` as the `Default` control | whole sets in both directions and no forbidden-name list of this crate's own invention: every declared field position against a 153-entry reviewed inventory that gives each one a purpose from a closed set, with exactly the thirteen fields of `PlanInputs` required to be public; every external `use` leaf against a 50-entry list; every function `comparison.rs` declares, private ones included, against a 24-entry list; the mastery vocabulary **derived from `P2-N2`'s own `ladder.rs` item declarations and `academic-domain`'s own `MasteryLevel` arms** rather than typed here, so a rung added there extends the guard; seven interior-mutability spellings and thirteen filesystem, clock, process and transport spellings as whole identifiers, and the two mutable-borrow spellings as text; `Default` refused anywhere in the package; and the canonical writers required unreachable from this package through a **declared edge of any kind**, walked over the workspace manifests rather than the source text | the package walk fails below twelve files, the field reader below one hundred positions, the manifest walk below forty packages, the module list below six entries, the ladder vocabulary below ten names and `MasteryLevel` below five rungs; and three controls, each requiring the same reader to find what it reports absent — at least four ladder names in `P2-C7`'s `proposed.rs`, at least two ambient names in the writer's `profile.rs`, and a `Default` in `P2-U8`'s bias draft — plus a closure control requiring `academic-projections` to be reported as reaching the writer |
| `deterministic_and_projected_are_separate_types_and_sections`, `hypothetical_and_actual_graduation_modes_are_distinct`, `workload_is_a_range_with_bias_metadata`, `end_of_term_calibration_emits_no_user_score` — `crates/what-if/tests/what_if.rs` | recursive, the whole `crates/what-if` package outside `tests`; plus five fixed reads — `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md` for section 22.1's two YAML blocks, 22.2's and 22.3's bullet lists, 22.4's table and workload sentence and 22.5's bullets; `crates/what-if/src/deterministic.rs`, `crates/what-if/src/projected.rs` and `crates/what-if/src/calibration.rs` for the field and method inventories of five of this crate's own types; and `crates/audit/src/verdict.rs` for `P2-U3`'s verdict vocabulary, with `crates/audit/src/engine.rs` as its control | the two lanes' struct bodies are read and required to name none of each other's types, in both directions with a control that requires each body to name its own; `ProjectedWorkload`'s whole method inventory against a four-entry list, so a `midpoint` added later fails as an entry nobody wrote down; `ModelCalibrationReport`'s and `CalibrationEntry`'s whole field inventories against reviewed lists; and `P2-U3`'s verdict vocabulary — **derived from that crate's own `pub enum`, `pub struct` and `pub type` declarations** — required absent from every product file of this package, beside the manifest walk that requires `academic-audit` unreachable through an edge of any kind | every design-document parse has a floor and every list comparison is ordered, so a bullet moved between section 22.2 and 22.3 fails rather than passing as a set; the verdict vocabulary fails below five names; and three controls — the lane bodies must each name their own five and seven types, `academic-export` must be reported as reaching the audit, and at least three verdict names must be found in `P2-U3`'s own engine |
| `crates/what-if/tests/support/mod.rs` | recursive, the whole `crates/what-if` package outside `tests`, and one non-recursive listing of `crates/` for every workspace manifest | not a scan of its own: it is the shared reader the two files above use — the package walk, the comment-and-literal stripper, the whole-identifier extractor, the block and field-declaration parsers, the field-position walker and the manifest closure walk — together with the synthetic fixtures, whose `ConfirmedSeat` is produced by driving `P2-U5`'s own `resolve` over an official registration-system reading and whose `BiasDisclosure` is built through `P2-U8`'s own draft | the package walk fails below twelve files and the field-position walk below one hundred positions, so a reader that silently found nothing is a failure in every test that uses it rather than a pass in all of them |
| `the_walk_reads_every_module_in_this_package`, `the_core_read_consults_no_connector`, `the_disclosure_is_bound_once`, `the_crate_reaches_only_the_declared_vocabulary`, `every_all_array_is_the_enum_it_names`, `every_public_signature_is_in_the_inventory`, `the_helpers_are_not_vacuous`, `this_scan_is_in_the_inventory` — `crates/integrations/tests/integration_scans.rs` | recursive, every `.rs` under `crates/integrations` outside `tests`, comments and literals stripped; plus fixed reads of this crate's three test files and of `docs/contracts/policy-source-scans.md` | whole-text pins on `IntegrationSurface::read_core` and on the whole set of identifiers its body spells, so a connector health check cannot enter the core read under any spelling; call-site counts by identifier on `bind_disclosure` (2), `proxy` (4), `transmit` (2) and `fleet` (4), plus a rule that the disclosure binding is the **first statement** of the one public transmit; whole-set comparisons in both directions of every absolute path root, every `use` item and every macro the product source spells, with `std::time`, `std::net`, `std::fs`, `std::env`, `std::process`, `std::io` and `std::thread` additionally refused by prefix on both sets; a comparison of every `pub const ALL` array against the variant list read out of the enum body it names, over the whole crate, with the set of types declaring an `ALL` pinned beside it so removing one is an extra key rather than an exemption; and a 120-entry inventory of every public signature, keyed on module, name and return type | `>= 80` public signatures; `>= 20` tests across the crate's suites; the module walk is compared whole against a six-file list, with a tripwire requiring every `mod name;` to be a file it read and `#[path]` refused outright |
| `core_graph_opens_with_every_connector_down`, `every_section_33_row_is_a_connector_kind`, `github_connector_is_read_only_and_scoped`, `a_blob_denial_has_no_payload_field`, `ide_adapter_performs_no_writes`, `ide_confirms_changed_scope_before_snapshot`, `assistant_receives_only_selected_ranges`, `generated_code_provenance_is_recorded`, `assistant_use_is_not_competency`, `calendar_payload_contains_no_grade_or_state`, `external_id_is_never_canonical`, `private_blob_egress_needs_a_second_grant`, `a_sync_conflict_preserves_both_sides`, `webhook_delivery_is_untrusted_and_builds_no_request` — `crates/integrations/tests/integrations.rs` | the same recursive product walk, plus fixed reads of `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md`, `crates/record/src/grade.rs`, `crates/domain/src/lib.rs` and every workspace `Cargo.toml` the closure walk reaches | section 33's table parsed back out of the design document and compared with `ConnectorKind::ALL` in both directions; the `GitHubOperation`, `HttpMethod`, `BlobVisibility`, `AssistantUse` and `EvidenceEligibility` variant lists read out of their own enums and compared with the `ALL` arrays; whole-set `(name, type)` field inventories for `ReadRequest`, `BlobDenial`, `GeneratedCode` and `CalendarPayload`, with the calendar's field **types** separately classified against a four-type admitted set so a `String`, an `f64` or a `MasteryLevel` fails whatever it is named; the whole method set of every `trait` this crate declares and of `GitHubConnector`, `IdeAdapter` and `IntegrationSurface`; the transitive product dependency closure read out of the workspace manifests and compared whole, with a control run over `crates/role-profile` that must find its competency edge; and a classification over every public signature requiring one that *returns* a `CanonicalRef` to have received one or to be an accessor on a type that holds one, with the holder set itself read out of the declarations | `>= 8` section 33 rows, `>= 25` grade and knowledge-state spellings, `>= 3` canonical-holder types, `>= 10` vocabulary names and `>= 20` IDE signatures; every byte-level absence has a control over a buffer that does carry what is being looked for |
| `crates/integrations/tests/support/mod.rs` | the recursive walk the two rows above share, declared by both with `mod support;` | not a scan itself: the walk, the extractors (`strip_non_code`, `absolute_paths`, `use_items`, `macros_spelled`, `public_signatures`, `public_signatures_with_owner`, `struct_fields`, `enum_variants`, `trait_methods`), and the synthetic ledger, keystore, workspace, transport and broker fixtures both suites build from | the floors its callers pass; `the_helpers_are_not_vacuous` re-runs every extractor against one sample it must match |
| `the_walk_reads_every_module_in_this_package`, `the_cs_map_crate_touches_no_file_and_no_socket`, `every_impl_header_in_this_crate_is_in_the_inventory`, `every_public_signature_is_in_the_inventory`, `the_producers_of_a_relevance_are_pinned`, `no_signature_names_both_a_relevance_and_a_mastery`, `no_function_returns_a_bare_change_origin`, `every_query_producer_returns_a_reveal`, `no_public_function_mutates_in_place`, `every_field_of_this_crate_is_in_the_inventory`, `this_scan_is_in_the_inventory`, `the_helpers_are_not_vacuous` — `crates/cs-map/tests/cs_map_scans.rs` | every `.rs` under `crates/cs-map/`, product tree and test tree; the `use`, path, macro, `impl`, signature and field comparisons read the product tree, and the forbidden-construct pass reads the test tree too | `P2-X5`'s crate boundary, closing **all three** bypass classes this run has measured. `P2-R2`'s is closed by three whole-set comparisons in both directions — every `use` item against a 27-entry inventory, every two-segment path reached through a crate root (one, `serde::Serializer`, and the `use` items are stripped first so an import is not counted as a reach), and every macro invoked (two, `matches` and `vec`). `P2-Y3`'s is closed by `every_impl_header_in_this_crate_is_in_the_inventory`, which pins all 30 `impl` headers of the package in both directions and then requires **none** of them to name `From`, `Into`, `Deref`, `DerefMut`, `AsRef`, `AsMut`, `Borrow`, `BorrowMut`, `TryFrom`, `TryInto`, `FromStr` or `FromIterator` for any type pair at all — stated over the whole inventory rather than over a list of pairs, so a conversion between two types nobody thought of is refused too; that is what keeps section 26.2's `opacity: 현재 lens relevance이지 mastery가 아님` and `P2-C6`'s refusal of `user scope change` as a `ChangeOrigin` properties of the declarations. `P2-N7`'s is closed by `every_public_signature_is_in_the_inventory`, which pins all 87 public signatures as `name | signature` in both directions, and the three rules drawn over that inventory: the whole set returning a `LensRelevance` **by value** is compared against two and both signatures are required to name no mastery type; the whole set naming both a relevance and a mastery is compared against the empty set, with both halves shown separately non-empty and the predicate shown to bite on a fragment that does map one to the other; and the whole set returning a bare `ChangeOrigin` is compared against the empty set while the `Option`-returning form is required to be exactly `change_origin`. Beside those, the whole set of public functions taking a `query: &str` is compared against one and pinned to return a `SearchReveal`; all 96 declared fields are compared in both directions and no `f32` or `f64` may be declared anywhere, so an opacity expressed as a fraction has no type to arrive in; a `mut self` scan refuses a mutable receiver anywhere in the package; and a 19-token forbidden-construct pass runs over every file of the package, tests included, with `Instant`, `SystemTime` and a bare `now` beside the filesystem and transport names because *this crate cannot ask what time it is* is what keeps a scrubber position a coordinate somebody chose | `>= 12` files in the package walk with every `mod` declaration required to resolve to a file the walk read and no `#[path]` anywhere, `>= 10` product files, `>= 70` public signatures, all 30 `impl` headers, all 96 fields and all 27 `use` items compared as whole sets in both directions; and controls on every helper — the `use` reader is required to ignore an identifier containing `use`, the path reader to ignore a middle segment and to find nothing once imports are stripped, the macro reader to ignore a keyword, the signature reader to find a `-> Result<Self, _>` shape, the field reader to find a named field and a wrapped one, and the `impl` reader to find a `From<MasteryFill> for LensRelevance` in a fragment that has one |
| `crates/cs-map/tests/cs_map.rs` | one fixed read of `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md` for nine readings, and one of `testdata/cs-map/landmarks.expected` and `testdata/cs-map/scrubber.expected` | not a source-text scan: `P2-X5`'s fourteen named acceptance tests plus eight more, over in-process values built from two synthetic fixtures. It reads the design document so that **nine** things are measured in both directions rather than restated — section 26.1's four zoom rows and their four distinct `감추는 것` cells, section 26.2's eight encoding bullets, section 25.3's ten lens names, section 25.3's `10–20` cluster range and its `수천 node가 아니라` clause, section 25.3's `YOU는 좌표 node가 아니라` sentence, section 26.3's one-base-two-overlay sentence together with its own example composition block, section 26.4's five focus bullets and its `cluster → path → node` sentence, section 26.5's three named transitions and its split-view sentence, and section 19's five back-quoted symbols. **Which channel each lens claims is derived from the document too**: each lens's own spec name is searched in each encoding bullet, and `claimed_channel` is required to be `Some` for exactly the four the document names and `None` for the six it does not, in both directions. `landmark_coordinates_stay_within_tolerance` compares sixteen landmark coordinates against the committed golden file, and `scrubber_matches_the_temporal_oracle` compares sixteen rendered lines against `tools/cs-map-scrubber-oracle.mjs`'s independent transcription in another language | four zoom labels, eight channel heads, ten lens names and five focus heads each compared position by position; the whole `base × overlay × overlay × overlay` product — 10,000 orderings, 30,000 insertions — enumerated with each verdict re-derived in the test; all 820 admissible compositions enumerated with both the colliding and the clear class required to be non-empty; twenty-one produced surfaces swept for `YOU`, each required to be non-empty first; five focus subgraphs compared node set **and** edge set in both directions; all nine section 30.2 statuses crossed with all twenty section 7.2 predicates; six mastery levels required to move the fill and not the opacity; and each of the five budget ceilings moved one past on its own and required to refuse by name |
| `tools/cs-map-scrubber-oracle.test.mjs` | none — it spawns `tools/cs-map-scrubber-oracle.mjs` and reads `testdata/cs-map/scrubber.expected` | not a source-text scan: it re-renders `P2-X5`'s independent scrubber oracle and requires the committed file to be byte-identical, then runs two negative controls — a reading whose two bitemporal axes disagree must give a different answer from one that collapses them, and the earliest reading must render empty. The oracle is a second transcription of the event table, of the identity derivation and of the projection algorithm, so the Rust comparison it backs is not the scrubber agreeing with itself | the `--check` run must exit zero, the rendered block must hold two lines per reading, and the two controls must each differ from the row they are compared against |
| `the_cs_map_lenses_are_the_crates_own`, `the_cs_map_regions_are_the_specifications`, `the_cs_map_route_opens_its_five_regions` — `packages/ui/src/cs-map.test.ts` | none — two fixed paths, `crates/cs-map/src/lens.rs` and `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md` | the arms of `MapLens::as_str` and the order of `MAP_LENSES` are parsed out of their own bodies — the match rather than the enum, so an arm renamed with a stale identifier fails on the identifier — and compared with `CS_MAP_LENSES` as sets in both directions and in the crate's order, which is section 25.3's. The five screen regions are compared against section 25.3's own bullet heads position by position, so a region invented in the shell has no bullet to match and one the document adds has no region. The rendered `/learn/concepts` view's section headings are then compared against the five in both directions, so a route that stopped supplying them fails as a missing set rather than as an empty frame | a raise if a signature or its closing brace is not found, `identifiers.size > 0`, a required single match of the CS map index destination, and a negative control: an arm the crate does not have reads as absent |
| `crates/build-learn/tests/build_learn_scans.rs` | recursive, the whole `crates/build-learn` package outside `tests`, plus one non-recursive sweep of every `.rs` of the package for nineteen forbidden constructs, and one fixed read of `docs/contracts/policy-source-scans.md` | `P2-R6`'s whole-set posture. `the_build_learn_crate_touches_no_file_and_no_socket` compares every `use` item, every two-segment path reached through a crate root and every macro invoked against pinned inventories in both directions, then sweeps every file of the package for the nineteen filesystem, clock, process and transport constructs. `every_impl_header_in_this_crate_is_in_the_inventory` pins all 55 `impl` headers and then refuses **every** folding trait — `Add`, `Sum`, `Mul`, `Deref`, `AsRef`, `Borrow`, `FromIterator`, `Index` and the rest — over the whole inventory for any type pair at all, which is the `From`-impl bypass `P2-Y3` measured and `P2-X5` found still open in `academic-blind-spot`. `every_public_signature_is_in_the_inventory` pins all 130 public signatures, which is the bypass `P2-N7` measured. `no_signature_folds_the_motivation_edges` compares the whole set of signatures naming a motivation type **and** returning a number against the empty set. `the_only_producer_of_a_technology_slate_takes_a_goal` and `the_only_producer_of_a_learning_item_takes_both` compare the whole set of producers of each against exactly one. `the_build_learn_crate_holds_no_phrase_list` reads the **unstripped** text and requires the plan validator's longest string literal to be a stable spelling | the impl, signature, use-item, reached-path and macro inventories are equalities in both directions rather than containments; the forbidden-construct pass fails below fourteen files and each of the nineteen constructs is required to be found by the same reader in a sample that spells it and *not* found inside a string literal; the motivation fold is an intersection with each half shown separately non-empty and the predicate shown to bite on a fragment that does fold; the numeric-field set is compared against exactly one entry; and `the_helpers_are_not_vacuous` re-runs every extractor against one sample it must match and one it must not |
| `crates/build-learn/tests/build_learn.rs` | one fixed read of `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md` for six readings | not a source-text scan: `P2-R6`'s nine named acceptance tests plus six more and `P2-RF19`'s `every_part_identifier_is_the_shape_this_crate_admits`, over in-process values built from `P2-N5`'s own fixture module. It reads the design document so that **six** things are measured in both directions rather than restated — section 20.1's six input kinds parsed out of its own sentence, its `ProjectGoal` block's four keys parsed out of its own YAML, section 20.2's five drawn readiness names and its six table rows with their six `뜻` cells, section 20.2's four checkpoint stages parsed out of its own example in the example's own order, section 20.3's three motivation labels parsed out of its own back-quoted sentence, and section 21.2's six status bullets. **The five-versus-six discrepancy is recorded rather than reconciled**: the drawn five are required to be an order-preserving injection into the six and the one row they do not name is pinned, so the test fails if the document stops saying either thing | six, four, five, six, four, three and six each compared position by position with the parsed list additionally required to be the length it is, so neither side can be empty; all six readiness rows required to be reached by a real `P2-N5` overlay; all six mapping statuses required to be reached by their own evidence; the two statuses that assert offering coverage required to refuse without it while the other four publish, so the refusal is about the two; and `a_fluent_lecture_list_plan_fails_validation` drives a plan spelled with none of the design document's words and requires the **same** defect set |
| `crates/build-learn/tests/support/mod.rs` | recursive only through `academic-gap`'s own fixture module, which it includes by `#[path]` | not a scan of its own: it is `P2-R6`'s fixture module — section 20.1's goal built through `normalize` and `ProjectGoal::state`, its two-decision AND/OR branch, real `P2-N5` `ConceptState` overlays over real `P2-N2` eligibility and real `P2-N3` bands, and `P2-U1` course revisions and offerings built through that crate's own drafts | the bands it builds are verified against what the fixture meant by `academic-gap`'s own `band_from`, so a fixture that stopped producing `STALE` fails there rather than making the readiness comparison vacuous |
| `every_field_type_in_this_crate_is_reviewed`, `the_public_signature_set_is_this`, `the_impl_blocks_naming_the_gate_types_are_these`, `no_type_this_crate_declares_is_named_by_another_crate`, `this_crate_reads_no_clock_and_no_environment`, `product_source_is_under_src_and_declares_no_path_module` — `crates/deletion/tests/deletion_scans.rs` | recursive, `crates/deletion/src`; plus one non-recursive listing of `crates/` and a recursive walk of each other crate's `src` for the name-collision pass | `P2-P2`'s crate boundary, as four whole-set comparisons in both directions and no forbidden-name list of this crate's invention. **The declared type of every named field** against a 47-entry reviewed list — declared *types*, not field names, because `P2-U8` measured a `Vec<u8>` called `excerpt` passing the shared `tools/secret-debug-policy.test.mjs` and `T197` measured that tool's text classification reaching only the two crates `TEXT_CLASSIFIED_CRATES` names, neither of them this one; the one array on the list is required to be the vault locator and nothing else, and `String`, the one type a caller could put anything into, is closed at the field *name* as well, against `action_id`, `detail` and `reason` with a stated reason for each. **Every public signature**, as file and collapsed head, against a 106-entry pin — `P2-N7` found five public functions handing a value out while spelling none of the names on a list — carrying two claims as properties of the set: none names a correction type *and* an incident state or closure, which is section 34.6's fifth principle, and none is byte-capable. **The whole `impl` header set** for the four gate types (`DeletionConfirmation`, `IncidentClosure`, `ProtectionDecision`, `DeletionTarget`) against a four-entry pin and required to hold no `From`, `Into`, `TryFrom`, `Deref`, `AsRef` or `Default`, because `P2-Y3` measured a conversion escaping every public-`fn` sweep. **Every type this crate declares** against every other crate's declarations, because `P2-N8` measured `academic-review`'s guard reporting five files of other crates through a shared `DimensionReading`. Then eight filesystem, clock and process spellings over every product file, the `#[path]` rule, and the product-source file set | the product walk fails below ten files for the clock pass; the type extractor fails below twenty declarations in this crate; and every extractor is exercised against a sample it must match — four field shapes including one split across lines, a multi-line `pub const fn`, a generic `impl` header — with two controls that require the same readers to find what they report absent: `DeletionPlan` must be found in `crates/retention/src` and must not be found here |
| `crates/deletion/tests/deletion.rs`, `crates/deletion/tests/deletion_faults.rs` | one fixed read of `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md` per case; `deletion_faults.rs` also writes and reads a temporary profile under the build lane's temporary directory, and spawns one child process | not source-text scans: `P2-P2`'s six named acceptance rows and faults `RB01`–`RB04`, over in-process values. The design document is read so that **two** things are measured in both directions rather than restated — section 32.10's seven-item derivative sentence, parsed and compared against `SPEC_DERIVATIVE_WORDS` in set *and* order, so "seven" is the document's number and not one this test remembers; and section 34.4's leak-row recovery cell, split and compared against `RecoveryStep::ALL`. The four protection policies each cite a sentence required to be in the document, and section 34.6's fifth principle is required verbatim. `dry_run_enumerates_every_derivative_class` drives **every** assignment of the three `ClassTargets` shapes to the seven classes — 2187 resolvers — rather than one case per class | every design-document parse fails if its clause is gone; the class sweep compares the node list against `DERIVATIVE_CLASSES` in registry order on all 2187 cases; `protected_artifact_returns_a_policy_reason` drives all four policies **and** an unprotected control, so the guard is not passing because everything refuses; `leak_incident_cannot_be_closed_by_claim_supersession` drives all three `CorrectionOutcome` arms and then records the four recovery steps one at a time, requiring `close` to keep refusing until the last, so it is not passing because `close` refuses unconditionally; and `a_confirmed_deletion_reaches_every_class_and_completes` is the completing control beside four fault rows that all assert an incomplete deletion |
| `no_clock_socket_or_file_reaches_this_crate`, `the_walk_reads_every_module_in_this_package`, `every_all_array_is_the_enum_it_names`, `the_public_surface_is_inventoried` — `crates/next-lecture/tests/next_lecture_scans.rs` | recursive, the whole `crates/next-lecture` package outside `tests`, plus one read of that package's own `src/lib.rs` | `P2-L6`'s absence half. The whole set of the package's `use` items, the whole set of the two-segment paths it reaches through a crate root with the `use` items removed, the whole set of the macros it invokes, and the whole set of its `impl` headers, each compared with a pinned inventory in **both** directions — the last of those is what `P2-Y3` measured a `From` conversion walking past a `pub fn` sweep with, and it is pinned for that reason. Then a whole-identifier pass for fifteen filesystem, clock, process and transport constructs; every `pub enum` in the package that declares an `ALL` compared against its own variant list; and every public signature classified by the type it sits on, with the free-function set named exactly | exactly `7` modules found by the walk and the same set `lib.rs` declares; `>= 3` `ALL` arrays; `>= 40` public signatures; `the_scan_helpers_are_not_vacuous` re-runs the `impl`-header reader, the enum reader and the `use`-stripper against samples each must match |
| `expected_concept_source_matrix`, `minimum_blocking_preparation`, `prep_uncertainty_factorization`, `morning_home_contract`, `only_the_prior_lecture_ending_cites_a_document_node`, `a_claim_quotes_the_material_it_names`, `an_extracted_claim_is_never_confirmed`, `the_next_lecture_crate_holds_no_phrase_list`, `propose_refuses_to_rank_and_refuses_to_invent`, `a_candidate_needs_the_graph_its_path_came_from`, `the_helpers_are_not_vacuous`, `the_fixture_is_section_12_7s_own_example` — `crates/next-lecture/tests/next_lecture.rs` | one fixed read of `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md`, for section 12.7's first and last sentences, its worked example, section 4's morning line and section 25.2's second numbered line; plus the same recursive product walk for the three rules that are statements about the public surface rather than about one call | `P2-L6`'s four named acceptance tests plus eight more, over in-process synthetic state whose claims are `P2-G5` `Proposal`s that `adjudicate` produced, whose descent is `P2-N5`'s own `search`, whose lecture evidence is a node of a document `P2-L4` produced, and whose bands are produced by `P2-N3`'s own `project`. It reads the design document so that seven extraction places and three uncertainty axes are **measured** in both directions rather than restated as numbers, and so that the morning's `1–3` is parsed out of two independent sentences, required to agree, and compared with `P2-X2`'s own constants. It reads this crate's own source for three claims a call cannot make: that exactly one public signature mentions `EpistemicStatus`, that no product file spells a variant of it other than `AiInferred`, that no public signature returns an `academic_knowledge_state` type, that every signature answering with a `ConfidencePermille` is an accessor on one of the three axis types or the claim, and that every non-ASCII literal in the package is a cell the design document writes | `>= 9` design-document cells found by the literal reader, `>= 30` public signatures found by the signature reader, `>= 2` parsed source cells, and the whole method inventory of the package required to match a pinned set exactly, with the two `contains` receivers enumerated |
| `crates/next-lecture/tests/support/mod.rs` | recursive, the whole `crates/next-lecture` package outside `tests`, one non-recursive read of the design document, and `P2-N5`'s own fixture module included by `#[path]` — which the inventory follows rather than treating as a read | not a scan itself: it is the shared reader the two rows above use — the package walk, the comment-and-literal stripper, the whole-identifier extractor, the `use`-item flattener, the public-signature reader with its owner resolution, and the enum and `ALL` readers — together with the synthetic materials, the adjudicated proposals over them and the claims extracted from those | the floors its callers pass; `the_helpers_are_not_vacuous` re-runs every extractor against one sample it must match, including the owner resolution, whose earlier form attributed a free function to the `impl` block that closed above it |
| `every_field_type_in_this_crate_is_reviewed`, `the_public_signature_set_is_this`, `the_impl_blocks_naming_the_gate_types_are_these`, `no_type_this_crate_declares_is_named_by_another_crate`, `every_test_this_crates_docs_cite_exists`, `this_crate_reads_no_clock_and_no_environment` — `crates/non-delegable/tests/non_delegable_scans.rs` | recursive, `crates/non-delegable/src`; plus one non-recursive listing of `crates/` and a recursive walk of each other crate's `src` for the name-collision pass | `P2-M4`'s crate boundary, as four whole-set comparisons in both directions and no forbidden-name list of this crate's invention. **The declared type of every named field** against an eight-entry reviewed list — declared *types*, not field names — required to hold no `Vec<u8`, `[u8`, `String`, `&str` or `AuthorityClass`, so this crate can carry a decision and not a payload byte, a free-text answer or a provenance class. **Every public signature**, as file and collapsed head, against a 24-entry pin, carrying two claims as properties of the set: every signature that can return a `DecisionEvent` or an `AuthorizedCommand` takes an `&Actor` or an `ActionCommand`, and none mints a `UserDecision`; `Self` in `decision.rs` is resolved to `DecisionEvent` by requiring that file to declare exactly one type, because without that the one producer whose return type is `Result<Self, _>` disappears from an inventory that still looks complete. **The whole `impl` header set** for the five gate types (`DecisionEvent`, `AuthorizedCommand`, `AuthorizedProposal`, `ActionCommand`, `NonDelegableAction`) against a five-entry pin and required to hold no `From`, `Into`, `TryFrom`, `Deref`, `AsRef` or `Default`, because `P2-X5` and `P2-Y3` both measured a `trait impl` escaping every public-`fn` sweep. **Every type this crate declares** against every other crate's declarations. **Every test name a product doc comment cites**, against the `fn` set of this crate's three test targets — this crate's own documentation cited three tests that were never written, which reads as proof and is not. Then seven filesystem, clock and process spellings over every product file | every extractor is exercised against a sample it must match — three field shapes including an enum struct variant and a tuple struct it must skip, a multi-line `pub const fn`, a generic `impl` header, and four tokens the citation rule must and must not match; the declared-type pass fails unless it finds `DecisionEvent` and `NonDelegableAction`; the citation pass fails unless it found all seven of the execution plan's named acceptance tests and read at least five citations; and the public-signature property is required to have found its two producers by name rather than being satisfied vacuously |
| `crates/non-delegable/tests/non_delegable.rs` | one fixed read of `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md` per subsection, through `crates/non-delegable/tests/support/mod.rs` | not a source-text scan. It parses section 27.1's table, section 27.2's bullets and section 27.4's four rows out of the design document and compares them against `CandidateGeneration`, `NonDelegableAction` and `academic_proposal::RiskTier` in both directions, so the compiled non-delegable set is the document's rather than one this crate remembers | the 27.1 extractor must read at least two rows and must not have run past its own subsection; the 27.4 labels are compared as an exact four-element list; and the claim that section 27 never names deletion is measured over the whole of section 27 rather than over 27.4 alone |
| `every_declaration_and_impl_in_this_crate_is_pinned` — `crates/transcript/tests/transcript_scans.rs` | recursive, every `.rs` file this package ships: the whole package outside `tests`, so `examples/` and `benches/` are read as well as `src/` — `S-12`'s row is the one about a walk that reads `<crate>/src` and stops seeing product-shaped code beside it | two whole sets compared in both directions: **every function declaration as a file, a visibility and a full signature**, and **every `impl` block header as a file and a header**. There is no forbidden-name list: a new function, a new method, a new inherent `impl`, a new trait `impl` and a new file all fail as an entry nobody wrote down. `P2-A3` measured why this crate needed one — a `trait impl` declares no `pub fn`, so a scan built on public signatures does not see it, and six of the eight `P2-U` crates had no inventory that would | `>= 10` files in the walk, 115 pinned declarations and 32 pinned impl headers, each compared as a whole list so an empty walk fails as a set of missing entries |
...
| `the_binary_runs_only_while_its_declaration_is_enforced`, `this_binary_is_bound_to_exactly_one_process_class` — `crates/repository-analyzer/tests/enforcement.rs`, `crates/capture-client/tests/enforcement.rs`, `crates/egress/tests/enforcement.rs`, `crates/export-job/tests/enforcement.rs` | none — one fixed read of each crate's own `src/main.rs`, beside launching that crate's built binary | **the whole of `src/main.rs`**, comments and blank lines removed, against a three-line constant: the `use`, the process-class `const`, and `academic_process_sandbox::class_main!(PROCESS_CLASS)`. It was two `contains` until `P2-RF31`, and `P2-A5`'s sixth audit put a live `std::net` name resolution above the sandbox entry with both substrings still present and the whole workspace green on both hosts. The crate declares no `fn main` at all now, so there is no statement position above the entry; the macro's argument is an `ident` fragment, so a statement written into the argument does not compile. Beside that, the launched binary either exits `0` with a receipt naming exactly the capabilities its class does not declare, or exits non-zero having written nothing to standard output | not a source-text scan: the source read is one file per crate and the load is carried by the exit status of a real process. Four of the six process classes; the two that are not here are the two whose declaration the boundary cannot enforce without contradicting it, and `a_declared_capability_is_not_defined_by_one_the_boundary_would_refuse` two rows down is what pins that pair |
...
| `the_items_tile_every_file_the_workspace_compiles`, `the_reader_refuses_an_item_form_it_has_no_rule_for`, `every_item_in_these_packages_is_pinned`, `the_pin_names_what_it_covers_and_what_it_does_not`, `every_item_that_reaches_a_closed_type_is_pinned`, `the_inventories_still_keyed_on_a_line_prefix_are_named`, `the_reach_readers_are_one_reader`, `the_shipped_binaries_are_the_ones_this_file_watches`, `no_process_class_binary_authors_a_statement_before_it_enters`, `every_path_a_shipped_binary_reaches_has_a_reason` — `crates/contracts/tests/item_inventory_scans.rs` | the compilation unit resolved by `crates/contracts/tests/support/mod.rs`, from every product target root through `mod`, `#[path]` and `include!`, recursively; plus every `crates/*/tests/**.rs` for the last row | the **items** a file holds, not the lines it spells. An item is read after its attributes, its visibility and its modifiers, and `mod`, `impl`, `trait` and `extern` blocks are descended into. Two properties carry the enumeration: a construct in item position that is none of the fourteen keywords and is not a macro invocation makes the reader return `Err`, and the extents it returns **tile** every product file — every non-whitespace character in exactly one **top-level** item — so an item it did not see **at top level** leaves a hole a test reads. An item written inside a leaf's body leaves no hole, because the leaf's own extent already covers it and the reader does not descend into a body: the tiling is total over the text, not over the items. What carries a body is the **sealed key** both pins compare — the declaration, plus a fingerprint of the item's own text **and of every literal value inside it, length-prefixed**, when the item holds no items of its own. The two halves are separate because the text half is the blanked view — a type named in a doc comment or in a quoted sample is not a route — and blanking erases a literal's content *and* its length, which `P2-A5`'s sixth audit measured by setting two domain-separation constants equal to one another with the whole workspace green on both hosts. On that, two pins: the whole item set of **every package the workspace compiles** -- package and key counts asserted by the inventory test, in `crates/contracts/tests/pinned-items/<package>.items`, one to a line, with the unpinned remainder asserted empty by name. There is no derivation in the selection role: `P2-RF30` derived it from a line-prefix arm and a closed-type arm, enumerated the 28 packages on neither, and `P2-A5`'s sixth audit then reached the network from inside one of the 28 -- `crates/repository-analyzer`, the analyzer process itself. Both derivations are kept as a **control** on the walk, so a truncated walk fails naming what it lost; and, per closed type, the items whose own text names it **or that sit inside something that does**, closed over aliasing to a fixed point so an item written against `type Removed = RestrictedOriginal;` is a route under its second name, which is the rule that fails naming the type. The owner half is what makes it a rule about a type rather than a signature: `pub fn bytes(&self) -> &[u8]` inside `impl ReleasableArtifact` names no type. Twelve closed types: the four `P2-A4` measured, plus `PromotionSet` and `MotivationDisplay`, which `P2-A5` measured folded into a scalar by a `pub const NAME: fn(&T) -> u32` that passed the whole workspace, plus `P2-X3`'s `RegistrationConfirmation`, `SecondaryPercentage`, `GpaFigure` and `PlanSnapshot`. `the_reach_readers_are_one_reader` holds the copies of the reach reader to one text, against `REACH_READERS` as a whole set in both directions, and requires every crate carrying one to drive the qualified-path form through it. Three further rules are over the eight crates that ship a `src/main.rs`, which the sixth audit found on no reach reader and outside the pin: the set of such crates as a whole set; the paths each reaches through a crate root, with a reason for each; and, for the four that enter the process sandbox, that their `src/main.rs` holds exactly three items and no `fn main` at all, because `academic_process_sandbox::class_main!` is the whole of `main` and there is therefore no statement position above the sandbox entry | `files.len() >= 500` product files, `items > 10_000`, `packages.len() >= 71` pinned packages -- the floor equals the workspace count now that the walk is the selection -- `total >= 17_000` pinned items and `files >= 30` in the shipped-binary walk, and a pin file that reads as empty is an error naming the path; the tiling is what fails if the reader stops short, because a file it read nothing from is all hole |
...
| `a_declared_capability_is_not_defined_by_one_the_boundary_would_refuse` — `crates/process-sandbox/tests/enforcement.rs` | `ProcessClass::ALL` × `ProcessCapability::ALL`, in memory | the obligation on the unenforced remainder. For every class, no capability it declares may have a `BrokerOnly` reason written in terms of a capability `refusals(class)` contains. Two pairs exist and are pinned whole — `CONNECTOR`/`StageExternalPayload` and `INDEXER`/`WriteSearchIndex`, both against `WriteStagedArtifact` — which is why those two boundaries are not the mechanical flag flip they look like: enforcing them would refuse each process the write its own declaration is defined by, vacuously, because both binaries are still the four-line stub. `EXPORT_JOB` has no pair, and that is why it is the one of the three `T229` enforced rather than recorded again: nothing it declares is defined in terms of a capability the boundary would refuse it | six classes read, and the scan is refused if no `BrokerOnly` reason names another capability at all |
| `the_walk_reads_every_module_in_this_package`, `the_dashboard_crate_touches_no_file_and_no_socket`, `every_capitalized_identifier_in_this_crate_is_in_the_inventory`, `planner_has_no_registration_endpoint`, `the_dashboard_surface_cannot_name_a_mastery`, `this_scan_is_in_the_inventory` — `crates/dashboard/tests/dashboard_scans.rs` | recursive, every `.rs` of the package, and `crates/record/src` for the control | four whole sets over this crate's product text, each compared in both directions and none of them a spelling list: every `use` item; every two-segment path spelled through a crate root, read by the canonical reach reader that `the_reach_readers_are_one_reader` holds this copy to; every macro invoked; and **every capitalized identifier**, which is the form a `pub static`, a `pub const NAME: fn(&T) -> u32`, a trait `impl`, an item inside a function body and an item a macro expands to all take. The registration absence is stated in four parts: the edge is declared and every ledger name is really spelled in `academic-record`'s own text; no product file spells `RegistrationConfirmation` or `SettledStatus`; the two planner files spell none of the six ledger names; and every occurrence of the two `timeline.rs` does spell is preceded by an ampersand. Deliberately **not** keyed on `pub fn `, `pub const fn ` or `impl ` | `files.len() >= 8` product files, `read >= 10` files on the forbidden-construct pass, `found.len() >= 100` names, `record_sources.len() >= 5`, and one control per net: the nineteen constructs are each found in a sample that spells one and not in a quoted one, the name reader is required to see all four bypass shapes and to report nothing for a local binding, and `<str as ::std::net::ToSocketAddrs>::to_socket_addrs(host)` is driven through this crate's own copy of the reach reader |
| `dashboard_shows_three_gpas_with_proof`, `audit_states_are_exactly_four`, `attempt_timeline_preserves_six_lifecycle_facets`, `planner_reevaluates_six_dimensions_on_drag`, `plan_snapshot_is_immutable_and_stale_marked`, `coverage_tabs_are_non_overlapping`, `catalog_and_review_are_separate_sections`, `percentage_is_secondary_with_breakdown`, `dashboard_no_composite`, `the_open_gates_are_section_38s_own` — `crates/dashboard/tests/dashboard.rs` | none — four fixed paths: `PERSONAL_ACADEMIC_CS_PROJECT_OS_END_STATE_DESIGN.md` for sections 25.4, 25.5, 25.6, 10, 36.9 and 38, `crates/dashboard/src/course.rs` for the catalogue identity's field list, `crates/review/src/scope.rs` for the review scope's, and `crates/dashboard/Cargo.toml` for the two edges the separation is a choice between | not a source-text scan in the policy sense. Eight enumerations are parsed out of the design document and compared with the crate's in both directions and in order; the audit-state parse then removes the four back-quoted words from the line and requires the residue to be punctuation. The two field-list reads compare a whole struct declaration against what section 25.6's own line names, in both directions, and require every review-dimension and aggregate spelling to be absent from the catalogue half and `CourseId` from the review half | no floor on a walk; each parsed list is compared with `assert_eq!` against a written-out enumeration, so a read that returned nothing fails as a set of missing entries |
| `the_academic_sections_are_the_crates_own`, `the_graduation_percentage_is_not_one_of_the_six_sections`, `every_academic_route_opens_its_own_sections` — `packages/ui/src/academic.test.ts` | none — four fixed paths under `crates/dashboard/src` | five enumerations read out of the crate's Rust source — the arms of an `id` or `spec_word` `match` and the order of the matching `ALL` constant — compared against `academic.ts`'s own lists in both directions and positionally, plus the crate's own `position` `match` against the shell's numbering. The shell list is written out rather than derived, so the two enumerations are independent | no floor on a walk; `identifiers.size > 0` per enumeration, and a control requiring an arm the crate does not have to read as `undefined` |
...
constant, and that constant must be one of that file's own reviewed calls; every
other `SYS_` name in the file must sit inside `denied_syscalls`, counted, which
closes the non-socket `SYS_` gap the previous rule left.

### The one computed syscall number the rule admits

The first-argument rule reads a path and refuses an expression, because a number
it cannot read is a number nobody reviewed. `P2-RF24` needs one number that
cannot be a path: the same syscall on the **x32 ABI**, which is the native number
with bit 30 set. Both seccomp backends make that call once, after the filter
installs, in order to observe that the filter refuses it —
`AUDIT_ARCH_X86_64` is the arch token for two ABIs, so a filter that checks only
the token leaves the second one unfiltered, and `P2-A5` measured a process that
reported `Seccomp: 2` completing a TCP handshake through it.

So the rule admits the prefix `X32_SYSCALL_BIT |` and nothing else computed. Two
things keep that from being a hole: the name after the prefix is reviewed like
any other, and a file that spells the prefix must contain
`const X32_SYSCALL_BIT: i64 = 0x4000_0000;` as whole text. Without the second,
`const X32_SYSCALL_BIT: i64 = 0;` would turn every x32 spelling back into a
native call the rule would still read as reviewed — the same reach the rule
exists to refuse. A bare `libc::syscall(41, …)` and a
`libc::syscall(0x40000029, …)` are refused exactly as before.

The rule also now splits the first argument on the closing parenthesis as well as
on the comma. A call with one argument has no comma, and reading to the next
comma anywhere in the file handed the rule text it could only fail on.
...
| S-21 | `academic_domain::Claim::validate_for_actor` and signed/ledger/store boundaries | **Implemented by T238 under resolved gate_7e4af1a2ad17.** A separate `DeterministicPrediction` actor carries engine name/version and frozen-input/rule-set digests in additive schema v4. The original result actor retains its existing authority. | The 360-cell matrix and signed metadata negatives enforce `Prediction + Prediction` only. Canonical author provenance prevents cross-class and cross-producer removal, scope/slot checks and user overrides survive replay, and official facts remain separate. Frozen v1/v2/v3 bytes remain unchanged; migration 0016 admits the actor only in the encrypted lane. Production data admission stays closed. |
| S-22 | `academic_audit::AuditError::RefusedWithNoCheck` — the totality arm of `DegreeAudit::assemble`'s verdict `match` | The arm exists because `IndeterminateVerdict::from_checks` answers `Option` and the `match` has to be total. It is **unreachable** on a published rule set and no test drives it. The argument, in full: `RuleSetDraft::publish` refuses a set with no rule, so `rules >= 1`; every rule with no recorded source page pushes `RuleSourceSpanAbsent`; `collect_missing` pushes a check for every `UNKNOWN` and every `CONFLICT` leaf; every pending transcript entry pushes `RecognitionUndecided`; every unresolved applicable conflict pushes `UnresolvedSourceConflict`; an unread conflict survey pushes `SourceConflictSurveyAbsent`; and both freshness arms push theirs. So a refusing gate always leaves a check behind. What used to stand here was `MissingCheck::SourceFreshnessPolicyAbsent`, which told a user who had recorded the freshness criterion to record it — a reason code that was false about the record. That is closed; what is open is that the replacement is a guard nothing exercises. | As soon as a fourth gate is added, as soon as an existing gate gains a refusal that pushes no check, or as soon as a `RuleSet` can be built by a route that does not go through `RuleSetDraft::publish` — any of the three makes the arm reachable, and it is the one path on which an audit returns an error rather than a verdict. `a_gate_that_refuses_always_names_a_check` drives the five routes that do reach a check; nothing drives the sixth because nothing can. Severity **P4**: it decides no verdict and it refuses rather than answering. |
| S-23 | `migrations/store/0015_phase2_requirement_rules.sql`'s `requirement_rule` projection, against `academic_requirement::publish::RuleSet::canonical_text` | The row carries `rule_id`, `rule_type` and `source_digest`. `rule_set_hash` is now taken over **every field of `RuleSet`** — its six own fields, all four of `OfficialSourceBinding`'s, and all three of `ExecutableRule`'s including the compiled body and the document identifier `source_rule` — so a rule set reconstructed from these rows cannot reproduce its own hash. This is not new: the body was already absent, and migration `0015`'s own preamble says the row carries the rule's identity, its kind and the digest of its source. This task widened the distance by one field (`source_rule`) and adds no column, because nothing in this repair reads or writes the store: `academic-requirement` declares no `academic-store` edge, `production_audit_no_llm` executes that closure, and the hash is taken over the in-memory value. | The moment anything replays a historical audit from the database rather than from a `RuleSet` value it already holds. Section 11.4's *과거 audit은 당시 입력과 rule hash로 재현한다* is satisfied today by `RuleSetLedger::by_hash` over values in memory; a restart that has to rebuild the ledger from `requirement_rule` would find the hash unreproducible. It belongs to whoever owns the store projection, not to `academic-requirement`. |
| S-24 | `academic_audit::AuditFacts::conflicts`, `Option<Vec<ConflictReference>>` | `None` is *nobody read the conflict store* and refuses a determination; `Some(vec![])` is *the store was read and held nothing* and admits one. What is open is that the second is still the **caller's assertion**: nothing here verifies that a store was consulted, only that the caller said so. That is the same standing `SourceFreshnessPolicy` has — the number is the user's and its absence is `INDETERMINATE` — and it is the honest limit of the repair: the engine can refuse an omission, it cannot detect a false claim. Before this, omission and observation were the same value and the gate issued a witness over an empty slice, which is what let `DETERMINATE POSSIBLE (conflict cases examined = 0)` be reached by passing no cases. | As soon as a second producer of `AuditFacts` exists. There is one today, and the conflict cases it carries come from `academic_ingestion::detect` through `ConflictReference::of`. A second caller that builds facts by hand could assert an empty survey it never made, and no check here would see it — closing that needs a typed value from the ingestion side saying the store was read, not a check on this side. Severity **P3**. |
| S-25 | `academic_record::policy::RuleBook::canonical_text` and `academic_record::grade::GradingScheme::canonical_text`, against the fields those types declare | Three `citation: String` fields — one on `GradingScheme`, one on `RepeatPolicyRow`, one on `ExternalGradePolicyRow` — are outside the rendering, so they are outside `RuleBook::digest`, which is the `RuleSetHash` the `GPA`, `CREDIT_ACCOUNTING` and `TRANSCRIPT_COVERAGE` engines replay against. This is the shape `T210` repaired in `academic_requirement::RuleSet::canonical_text`, one crate out: a hash rendered over the fields somebody judged load-bearing rather than over the value. **It decides nothing today**, and that is measured rather than assumed: `.citation()` has no caller anywhere in `crates/` — the accessor is declared in `grade.rs` and read by no file — so two rule books differing only in a citation produce identical proof trees and identical `EngineOutcome` bytes. It is recorded rather than fixed because closing it re-emits three engines' fixture directories in a crate `T210` does not own; `P2-U4` does. | The moment anything reads a citation into a proof leaf, an explanation snapshot or an export. Section 11.3 already makes the citation part of the proof for `academic-audit`'s rules — `RuleSourceSpan::canonical_text` renders all five of its fields for exactly that reason — so a record-side proof leaf that quotes its policy row's source is the edit that makes two different rule books answer differently under one hash. Severity **P3** until then. |
| S-26 | `academic_audit::RuleSourceSpan::source_digest`, against `academic_requirement::ExecutableRule::source_digest` | **Closed by `P2-RF27`.** The span's own documentation calls it *the byte span of the paragraph inside the official snapshot whose digest the rule already carries*, and nothing compared the two: `DegreeAudit::assemble` took the span out of `AuditFacts::sources` by rule identifier and never read the rule's digest at all, so a leaf could cite a paragraph of a document its rule was not read from. Measured on this tree before the repair: an index built with a digest no published rule rests on gave `DETERMINATE NOT_POSSIBLE`, **no outstanding check**, and thirteen leaves citing that other snapshot. `RuleSourceIndex::span` is gone and there is no accessor that hands out a span for a bare identifier: `RuleSourceIndex::placement` takes the published set and the comparison is the only way to reach a span, its third arm is `MissingCheck::RuleSourceSpanIsAnotherDocument`, and the rule is left unevaluated. The same input now refuses all eight rules and cites nothing, one moved placement refuses exactly one, and the truthful placements still publish thirteen citing leaves. | Nothing. What is left is one step out and is `S-27`: the digest the rule itself rests on is the reviewer's declaration, and the leaf is now bound to that rather than to the document. Severity **P2** while it was open. |
| S-27 | `academic_requirement::ExecutableRule::source_digest`, against the document the rules were parsed out of | Nothing checks that a rule's declared source snapshot is the document it was published from. `academic_ingestion::publish` carries each rule's own `text_digest` and **not** the sealed `RawSnapshot::digest`, so `PublishedRules` cannot state which document it is and `RuleSetDraft::include` has nothing to compare a declared `source_digest` against — the value arrives from `RuleCandidate::extracted` and is believed. Measured on this tree: the fixture document's bytes hash to `sha256:9e3d…f1be`, the rules published from it rest on `sha256:8709…a642`, and a set whose rule declares `sha256:7342…94d1` — a digest of a string no document has — publishes. It is `S-24`'s sentence one step past `S-26`: `P2-RF27` bound a leaf's citation to its rule's declaration, and this is what binds the declaration to the document. | Now, for any consumer that reads a leaf's `@text/` locator as naming a real document. Closing it is the `P2-RF26` shape — publication stops dropping a value it has — but it moves every fixture's declared digest and therefore `ruleset.txt`, the `rule_set_hash`, and the whole `graduation_audit` corpus, which is why `P2-RF27` recorded it instead. Severity **P2**. |
...
`crates/audit/tests/audit_scans.rs` holds eleven scans. Four read the design
document and compare a vocabulary against it in both directions; three sweep the
crate's own product source as a whole set; one is the tripwire the other ten rest
on; one compares the whole `Default` set; one compares every declaration and
every `impl` header in the package against two pinned lists; and one compares the
scans this file declares against the two documents that enumerate them.

The count is measured rather than asserted:
`the_scans_these_documents_enumerate_are_the_ones_this_file_holds` reads the
number word and the two enumerations out of this document and out of
[graduation audit](graduation-audit.md) and compares them against the `#[test]`
functions the file declares, in both directions. `P2-A3`'s second audit found
this paragraph saying nine while the injection matrix above named ten, so the
paragraph is now a claim that fails when it stops being true.
...
| `every_declaration_and_impl_in_this_crate_is_pinned` | **every function declaration as a visibility and a full signature** and **every `impl` block header** in the whole package — everything outside `tests`, so `examples/` is read too — against two pinned lists, in both directions | a new function, a new method, a new inherent `impl` and a new trait `impl` all fail as an entry nobody wrote down, whatever they are called. Its walk keeps `*.rs` only, which is `P2-A3`'s **F-A** and is a workspace-wide finding rather than this crate's |
| `the_scans_these_documents_enumerate_are_the_ones_this_file_holds` | the `#[test]` functions this file declares against the number word and the two enumerations in this document, and against the number word in [graduation audit](graduation-audit.md), in both directions | a scan added without a row here fails; a row naming a scan the file does not declare fails; a number word that stops matching the count fails |
...
`S-21` is implemented by the resolved separate-actor choice in
`gate_7e4af1a2ad17`: deterministic forecast provenance uses schema v4 while the
original result actor, official-source producers and confirmed planning seats
keep their existing boundaries. The contract and acceptance tests above record
the behavior; earlier attribution of a ModelRun-only rule to ADR-003 remains
...
| | |
|---|---|
| crate directories under `crates/` | **71** |
| packages whose own inventory is keyed on a line prefix | **43**, in 49 files |
| packages carrying a whole-set item pin | **43 of 43** |
| items pinned | **11748** |
| of those, keys carrying a body fingerprint | **9860**; the other **1888** are containers |
| crates with **no** item pin | **28** |
| closed types pinned workspace-wide | **12** (4 from `P2-A4`, 2 from `P2-A5`, 4 from `P2-X3`, 2 from `P2-RF30`) |
| copies of the reach reader, held to one text | **18** (`P2-A5` counted 14; `crates/*/tests/*.rs` does not reach `tests/support/mod.rs`; `P2-X3` added the seventeenth and `P2-RF30` the eighteenth) |

**Every number in this table is asserted.** `P2-A5`'s fifth audit found this
page's neighbour in `item_inventory_scans.rs` claiming 45 unpinned crates and 68
in the workspace where the tree held 47 and 70, and recorded that it survived
because nothing asserted either number. It then moved twice more, under `P2-RF29`
and `P2-X3`, while the prose stood still — this table said 70 crate directories
and 45 without a pin against a tree holding 71 and 46.
`the_pin_names_what_it_covers_and_what_it_does_not` compares the workspace's
package count, the derived count and the pin directory against each other, and
takes the remainder as a **subtraction** rather than as a fourth number somebody
keeps in step by hand.

No crate needs hand work, because the pin is keyed on nothing a crate can differ
in -- it is that package's item set, and the item reader is one reader.

### The 28 that have no item pin, by name

`admission`, `capture-client`, `cli`, `connector`, `contracts`, `core`,
`crypto`, `daemon`, `desktop`, `domain`, `egress`, `export-job`, `indexer`,
`ledger`, `policy`, `portability`, `projections`, `recovery`,
`repository-analyzer`, `retention`, `rpc`, `scenario`, `store`,
`store-platform`, `test-support`, `transcript`, `vault`, `worker`.

Eighteen crates left this list in `P2-RF30` and two more in `P2-X3` before it.

**Why this list is shorter, and what the old reason for its length was.** The
page used to say of the crates on it: *"These crates keep no whole-set inventory
of their own that a line prefix could blind, which is why they are not on the
derivation: there is no false sentence to make true."*

**That was false, and `P2-A5`'s fifth audit measured it false.** One method added
inside the existing `impl StageEvidence` block of `crates/competency` — spelling
none of the names that crate's guards list, adding no `use` item, no field and no
re-export — filled a personal competency cell at the `USED` stage from a snapshot
identifier and a classification token alone, and passed the whole workspace on
both hosts. `crates/competency` is `P2-R5`'s only consumer on the
personal-competency path and its module documentation opens with the claim that
there are two doors into stage evidence and no third. It was on this list. The
identical shape in a pinned package was caught by name, so **the attribution was
the pin's scope, not the item's form**.

The same audit measured a second reason the list was too long: the line-prefix
arm searched for three assembled literals, so `consent`, `curriculum` and
`untrusted-content`, which write their `impl` prefix with no trailing space, and
`what-if`, which strips a prefix rather than testing one, held the shape and were
outside the arm anyway. `PINNED_PACKAGE_FLOOR` was 23 and the derivation found
exactly 23, so the floor could not see the miss either.

Both are repaired without inventing a third derivation. `competency` enters
through **both** existing arms: declaring `StageEvidence` and `PromotingEvidence`
in `CLOSED_TYPES` puts it on the arm `P2-X3` built for exactly this — *where a
package says that its own surface is the subject of a contract sentence* — and
the line-prefix detector, asked about the literal's **first word** instead of
about three spellings, reports it as well. The arm grew from 23 packages to 43.

**What is still true of the 28.** An item added to any of them is not an extra
key anywhere.
...
reader has no rule for. So a route from one of the 28 **into** a closed type is
caught today; an item that folds a type one of the 28 owns is not.

The moment one of the 28 grows a contract sentence about its own surface --
"the only accessor", "no signature folds these" -- it needs a pin, and the
line-prefix derivation will not add one for it, because it keys on the collector
rather than on the sentence. That was the seam: a crate that writes such a
sentence **as an item-based rule from the start** never appears on the
line-prefix list and never got a pin.

`P2-X3` closed the seam in the rule rather than working around it. The
pinned-package set is the **union of two derivations**: the packages that key an
inventory on a line prefix, and the packages that own a type in `CLOSED_TYPES`.
Declaring a closed type is how a package says its own surface is the subject of a
contract sentence, and it pins that package's whole item set.

`P2-RF30` is what that arm was built for. `crates/competency` held the sentence
`P2-A5` measured broken — *two doors and there is no third* — keyed no inventory
on a line prefix that `T227`'s detector could see, and owned no closed type, so
it was outside both arms. Declaring `StageEvidence` and `PromotingEvidence` puts
it on the second, and normalising the first arm's detector puts it on that one
too; either alone is enough, and the arm grew from 23 packages to 43 because the
detector stopped keying on three spellings.

**What is still not read is the sentence itself.** A crate that writes one, keys
no line-prefix collector and declares no closed type still gets no pin, and
nothing in this repository reads sentences. `crates/policy` is that shape today,
measured rather than argued: `#[derive(Debug)] pub struct SurfaceProbe;`
appended to `crates/policy/src/lib.rs` leaves `academic-policy` and
`academic-contracts` at **67 passed, 0 failed, exit 0**. The 28 above are the
list of crates where that is true, and the only closure over it that needs no
detector at all is to pin every package the workspace compiles — 71 files
instead of 43, and every body edit anywhere a line in a pin. `P2-RF30` did not
make that trade and records it here as the one that remains.

### What the eighteen copied reach readers hold now
...
They are eighteen copies. The eighteenth is `P2-R1`, and `P2-A5`'s fifth audit
is what found it missing: `REACH_READERS` did not contain `crates/repository`,
`T226` had recorded that crate as using a different reader, and **it used none**.
`the_crate_touches_the_filesystem_only_to_read_it` compared every `fs::` name
against a three-entry list and every `use` item against a fourteen-entry list and
had no third comparison, so a fully-qualified inline `std::net` path inside
`SourceKind::of_path` — a live function on the capture path — left both that test
and `analyzer_cannot_mutate_source_or_open_a_socket` at 11 ok apiece, and none of
that second test's three levels is about this crate's own source.

Since `P2-RF29` the sealed key does notice such an injection, because
`of_path`'s body text moves — but it notices it in another crate's test binary,
as *"`academic-repository` compiles items nobody wrote down"*, which says that
some text changed and not that this crate reached the network.
`REACHED_PATHS` is a whole set of three entries with their reasons now, and the
failure names `std::net`, in the suite where the claim lives. Measured on
`b31cc27` before the repair and on this branch after: three forms —
`<str as ::std::net::ToSocketAddrs>::to_socket_addrs`, `::std::env::var` and
`::std::process::id`, none spelling an `fs::` name or adding a `use` item — each
the only failure of eleven, each naming its own crate root.

A dev-dependency crate would make them one, and would add an edge to the
dependency closure of eighteen crates -- the closure that
`tools/phase1-scaffold-policy.test.mjs`'s dependency map, its acyclic graph and
each crate's `USE_ITEMS` inventory exist to hold still. Paying in the thing being
protected to deduplicate the thing protecting it is the wrong trade. What
replaces one copy is `the_reach_readers_are_one_reader`: the bodies are compared
against each other with the crate-root list removed, so eighteen copies are one
text, and every carrier crate is required to hold the driving control. One driven
copy plus textual identity is the same guarantee as one function, and a
nineteenth copy that arrives with the old body fails by name. The eighteenth was
not written by hand: it was taken whole out of
`crates/repository-analysis/tests/analysis_scans.rs`, and that rule is what says
so.
...
`P2-RF30` added seven rows. Every "before" was measured on a second lane
extracted from `git archive b31cc27`, with the injected files' git object hashes
checked against the commit first; every "after" is its own build on the branch.

| # | injection | before the repair | after |
|---|---|---|---|
| 12 | `pub fn of_snapshot_reading(id, stage, snapshot: &str, concept: &str)` inside the existing `impl StageEvidence` block of `crates/competency` — no forbidden token, no `use` item, no field, no re-export | **exit 0**, `item_inventory_scans` 6 ok: the crate was on neither derivation | `every_item_in_these_packages_is_pinned` FAILED naming the item **and** `every_item_that_reaches_a_closed_type_is_pinned` FAILED naming `StageEvidence` and quoting the contract; 5 passed, 2 failed |
| 13 | the same door written as an `impl StageEvidence` block **inside the body** of `StageEvidence::id` — globally effective Rust the item reader does not descend into | **exit 0** on the same lane, riding in the same build as row 12 | the same two failures. `P2-RF29`'s sealed key is what carries the body once the package is pinned; the closed type is what names the route |
| 14 | `pub const RAW_LEN: fn(&str) -> usize` at module level in `crates/untrusted-content` — one of the four packages the three-literal detector missed | (unpinned: nothing to fail) | `every_item_in_these_packages_is_pinned` FAILED naming `const RAW_LEN: fn(&str) -> usize`; the only failure of seven. The first form of this injection did not compile — it displaced `admit`'s doc comment under `#![deny(missing_docs)]` — and an injection that does not compile is not evidence |
| 15 | `#[derive(Debug)] pub struct SurfaceProbe;` in `crates/policy`, a package on **neither** derivation | **exit 0** | **still exit 0**, 67 passed / 0 failed. This is the residue, measured rather than argued: the 28 crates on neither arm are where an added item is an extra key nowhere. The first form of it did not compile either, under `-D missing-debug-implementations` |
| 16 | `<str as ::std::net::ToSocketAddrs>::to_socket_addrs("example.invalid:80")` in `SourceKind::of_path`, `crates/repository/src/snapshot.rs` | `repository_scans` **11 ok**, `snapshot` **11 ok** — including `analyzer_cannot_mutate_source_or_open_a_socket`; the only thing that fires is the sealed-key pin in another crate's binary, saying an item's text changed | `the_crate_touches_the_filesystem_only_to_read_it` FAILED naming `std::net` as a path outside the inventory; the only failure of eleven |
| 17 | `::std::env::var("PATH")` and, in a second build, `::std::process::id()` at the same site | -- | the same test FAILED naming `std::env` and `std::process` in turn, each the only failure of eleven |
| 18 | injection K: the `BPF_JGE` floor removed and `x32_answer()` clamped, then separately the floor's `k` replaced with `u32::MAX` | 20 passed, `native.rs` 4 passed, x32 ABI open | `the_x32_floor_is_pinned_as_the_bytes_the_kernel_reads` FAILED on the program's shape and, for the second form, on the eight bytes; the only failure in the package either way |

### Two findings recorded rather than repaired

**Injection K is closed by `P2-RF30`.** The paragraph below is what three audits
recorded; the close it names is made. `refuse_every_socket` assembled the filter
and installed it in one function, so the program was never a value a test could
read. The assembly is split into `socket_filter_program`, and
`the_x32_floor_is_pinned_as_the_bytes_the_kernel_reads` reads it: the program is
`denied_syscalls().len() + 7` instructions, instruction 4 encodes little-endian
to `35 00 0e 00 00 00 00 40` — `BPF_JMP | BPF_JGE | BPF_K`, a jump past all
thirteen denied comparisons and the allow, no fall-through, and
`X32_SYSCALL_BIT` — and it is the only unsigned jump in the program. The eight
bytes are spelled out from the four `#[repr(C)]` fields rather than transmuted,
so the pin is over an encoding a reader can check against the seccomp ABI.
x86_64 only, because the x32 ABI is.

Both halves are driven in the WSL lane. Under the whole of injection K the
package is 20 passed with `native.rs` 4 passed — the audit's number, with the
x32 ABI open — and the byte pin is the only failure. Replacing the floor's `k`
with `u32::MAX`, which leaves the length and the opcode intact and lets every
x32 number through, fails on the bytes rather than on the shape: `[53, 0, 14, 0,
255, 255, 255, 255]` against `[53, 0, 14, 0, 0, 0, 0, 64]`. A length pin alone
would have accepted a floor nobody can reach.

The original record follows.

**Injection K persisted for three audits and completed a TCP handshake.** `P2-A5` removed the
...
## What the `P2-X3` rollout closed

`P2-X3` is the first crate in this repository to write a contract sentence about
its own surface **as an item-based rule from the start**. The seam the `P2-RF28`
rollout recorded is exactly that case: `academic-dashboard` keys no inventory on
a line prefix, so `the_inventories_still_keyed_on_a_line_prefix_are_named` never
reports it, and the derivation that follows from that test would never have
given it a pin.

### What was widened, and what it costs

`every_item_in_these_packages_is_pinned` now derives its package list from
`packages_that_need_an_item_pin`, which is the **union** of two derived sets:

| source | what it means | packages |
|---|---|---|
| a file the line-prefix derivation reports | the contract rests on a reader that cannot see an item form | 23, unchanged |
| a type in `CLOSED_TYPES` | the package says its own surface is the subject of a contract sentence | `student-voice`, `capture-gate`, `repository-competency`, `build-learn` (all already on the first list), plus `record` and `dashboard` |

Neither half is written down. The second is read out of `CLOSED_TYPES`, which is
where a closed type is declared anyway, so a package that declares one and has no
pin file fails by name rather than being counted as covered.

**The cost is `record`.** Adding `RegistrationConfirmation` as a closed type put
`academic-record` on the union, and its whole item set — 397 keys — is now
pinned. That is a real strengthening of a crate `P2-X3` does not own, and it is
deliberate: `gpa-and-attempts.md` already says *two constructors, and neither
takes a status the caller chose*, which is a sentence about that crate's own
surface, and until now nothing held the surface whole.

**Both new pins are in `P2-RF29`'s sealed-key form.** 220 of `dashboard`'s 264 keys and 343 of
`record`'s 397 carry a body fingerprint; the rest are containers whose contents are enumerated
as items of their own. So an `impl` written into the body of an existing method of either
package — the shape `P2-A4`'s third audit put past the old `key` form — moves a line here.

**What the union does not reach, measured rather than assumed.** `crates/competency` is **not**
in it: it keys no inventory on a line prefix, and no type it owns is in `CLOSED_TYPES` —
`StageEvidence` appears in `item_inventory_scans.rs` zero times. The derived set is exactly the
25 file names under `pinned-items/`, and the rule compares that directory against the
derivation in both directions, so the listing *is* the measurement.

**What is still open.** A crate that writes such a sentence and declares no
closed type still gets no pin. Reading the sentence is what would close that, and
nothing here reads sentences.

### The four closed types, and what each one is for

| type | package | the sentence the pin makes true |
|---|---|---|
| `RegistrationConfirmation` | `record` | section 25.5's *사용자의 실제 수강신청을 자동 수행하지 않는다*, and `P2-M4`'s non-delegable confirmation. A route from any crate fails **by name** |
| `SecondaryPercentage` | `dashboard` | section 25.4's *상세 breakdown이 항상 붙는다*: `over` is the one producer and takes the breakdown by value |
| `GpaFigure` | `dashboard` | section 25.4's *각 계산 proof* and section 10's *같은 카드의 한 score로 합치지 않는다* |
| `PlanSnapshot` | `dashboard` | section 25.5's *무엇이 stale해졌는지만 표시한다* |

`academic-dashboard` links `academic-record`, so `RegistrationConfirmation` is
nameable from the crate whose contract is that it does not name it. That is the
point of the edge: an absence over a type a crate cannot name is not an absence.

### The two shapes this task found in its own suite

**A compile-fail probe that produced no diagnostic.** `E0451` — *fields … are
private* — is reported by the privacy pass, and **that pass does not run once
type checking has failed.** Two of the four cases in the first version of this
suite put a struct-literal probe beside method probes for the same type, and the
committed `.stderr` for `a_plan_snapshot_cannot_be_restated_in_place` and
`a_percentage_is_not_built_from_a_number` held only the method errors. The
probes were carrying no load, and reading the committed diagnostic is what found
it. `GpaFigure`'s did report, and only because it omitted a field, which is a
resolution-phase error rather than a privacy-phase one. The three privacy probes
are now alone in `no_academic_surface_type_has_a_public_field.rs`, where each
appears in the diagnostic.

**A compile-fail case whose errors were about `Result`.** The same two cases
built their subject with a constructor that returns `Result`, so the diagnostics
read *no method named `set_label` found for enum `Result<T, E>`* and said nothing
about `PlanSnapshot` at all. Each case now takes its subject as a parameter.
