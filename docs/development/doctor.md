...
`academic doctor --format human|json` (or `pnpm run doctor`) is a local, privacy-safe prerequisite check. It records tool version, resolved executable path when available, whether the pin is supported, and a reproducible remediation command. Use `pnpm run doctor` exactly; bare `pnpm doctor` is pnpm's own network-capable diagnostic command, not this repository script. The wrapper invokes Cargo with both `--locked` and `--offline`, so it performs no dependency resolution over the network.

The command does not inspect documents, repositories outside the checkout, credentials, audio devices, browser state, cloud accounts, or network endpoints. It performs no network request. Because Cargo is forced offline, `pnpm run doctor` requires a populated Cargo dependency/index cache from a prior bootstrap or an equivalent completed `cargo fetch --locked`; with an empty cache it fails locally instead of contacting a registry. A successful result means only that Phase 0 build tools match their pins; it does not mean ADR-002 encrypted storage or later security gates are accepted.
