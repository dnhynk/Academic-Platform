...
- `Cargo.lock` and `pnpm-lock.yaml` are committed. Before any fetch/install/build/test, dependency-free checked-in TOML/YAML parsers structurally inspect Cargo package sources and pnpm importers, packages, snapshots, and default/named catalogs. Cargo Git sources, malformed/duplicate lock structures, pnpm Git source types/repository fields/SSH/HTTPS/Git/file forms/hosted shorthands, and insecure HTTP tarballs are rejected independent of legal quote, spacing, multiline, block, or flow serialization; registry, workspace/link, and ordinary local-file dependencies remain allowed. Committed Cargo and pnpm allow/reject corpora probe the accepted and denied encodings.
