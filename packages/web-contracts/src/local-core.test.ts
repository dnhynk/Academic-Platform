...
void test("local core Proto tags, oneofs, and reserved bands are drift-pinned", async () => {
  ...
  for (const messageName of ["DetailRequestFrame", "DetailResponseFrame"]) {
    const detailFrame = root.lookupType(`academic.v1.${messageName}`);
    assert.deepEqual(
      Object.fromEntries(
        Object.entries(detailFrame.fields).map(([name, field]) => [
          name,
          { id: field.id, type: field.type },
        ]),
      ),
      { canonical_json: { id: 1, type: "bytes" } },
    );
    assert.deepEqual(detailFrame.reserved, [[2, 15]]);
  }
